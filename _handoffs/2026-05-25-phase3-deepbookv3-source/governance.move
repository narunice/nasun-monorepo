...
/// Event emitted when trade parameters are set by admin (bypassing the
/// proposal/vote flow). Distinct from `TradeParamsUpdateEvent` for indexer
/// auditability.
public struct TradeParamsAdminUpdate has copy, drop {
    taker_fee: u64,
    maker_fee: u64,
    stake_required: u64,
    epoch: u64,
...
/// Set trade params via admin path (bypasses the proposal/vote flow).
/// Caller is gated by `DeepbookAdminCap` in the public wrapper. Applies
/// immediately AND persists as next_trade_params, so the change survives
/// the next epoch transition. Whitelisted pools still cannot change.
public(package) fun set_trade_params_by_admin(
    self: &mut Governance,
    taker_fee: u64,
    maker_fee: u64,
    stake_required: u64,
    ...
    // Admin path intentionally bypasses the whitelisted-pool guard: in Pado's
    // forked deepbook all production pools are flagged whitelisted (= managed
    // by Pado, not by external LP governance). The `EWhitelistedPoolCannotChange`
    // assert in `add_proposal` exists to block governance proposals; the
    // admin cap holder is precisely the entity that should be able to
    // re-tune baseline fees on those pools.
    assert!(taker_fee % FEE_MULTIPLE == 0, EInvalidTakerFee);
    assert!(maker_fee % FEE_MULTIPLE == 0, EInvalidMakerFee);
    if (self.stable) {
        assert!(taker_fee >= MIN_TAKER_STABLE && taker_fee <= MAX_TAKER_STABLE, EInvalidTakerFee);
    ...
    self.next_trade_params = params;

    event::emit(TradeParamsAdminUpdate {
        taker_fee,
        maker_fee,
        stake_required,
        epoch: ctx.epoch(),
