...
## 2026-04-13 D+1 헬스체크 및 미세 개선 (plan: adaptive-wandering-lecun)

### D+1 발견 및 수정
1. **pg_dump cron 04-12 18:00 UTC run 실패 확인** — 원인: 신규 `snapshot_change_log` + `_tmp_snapshot_*` 테이블에 `sui_indexer` 권한 없음. `permission denied` → gzip 빈 출력
2. **수정 완료**: `GRANT SELECT ON snapshot_change_log, _tmp_* TO sui_indexer` + `GRANT SELECT ON ALL SEQUENCES ...` + `ALTER DEFAULT PRIVILEGES ... GRANT SELECT ON TABLES TO sui_indexer` (향후 테이블 자동 grant 예방)
3. **오늘치 pg_dump 수동 재실행**: `nasun_points-20260413.sql.gz` 199MB 생성
4. **backup-snapshots cron 시간 조정**: `0 1 UTC` → `0 3 UTC` (daily snapshot 생성 후 fire)
5. **임시 테이블 cleanup**: `_tmp_snapshot_totals_20260412`, `_tmp_snapshot_backup_penalty_20260410` drop (오늘 dump에 보존됨)

### 비긴급 개선 (plan adaptive-wandering-lecun 실행)
1. **nasun-chat-server ecosystem.config.cjs에 `log_date_format` 추가** — pm2 logs에 라인별 timestamp prefix 확보 (재시작 시점 추적 가능)
2. **진단 인프라 도입**: systemd user timer `pm2-trace.timer` (6h 주기) → `~/pm2-restart-trace.log`에 pm2.log + dmesg OOM + pm2 jlist summary 누적 수집. `loginctl enable-linger ec2-user`로 세션 독립 실행

### 초기 trace 수집 결과 (첫 run)
**nasun-chat-server 재시작 패턴 = SIGINT exit code 0**:
- 2026-04-13 01:27:18, 05:46:45 모두 SIGINT 종료 → PM2 autorestart
- OOM 이벤트 0건 (dmesg clean)
- "Process 29 in a stopped status, starting it" 메시지 관찰 → max_restarts 초과 후 PM2가 stopped 처리, 외부 트리거로 재시작
- 의심: 외부 pm2 stop/start 명령 또는 max_restarts 히트

### 1주일 후 재평가 (2026-04-20 예정)
- `~/pm2-restart-trace.log` 누적 데이터로 nasun-chat-server SIGINT 트리거 범인 식별
- explorer-api `CONNECTION_ENDED` 분포 (burst vs steady) 확인
- 결정: 코드 수정 (scanner try/catch / chat-server handler) vs 운영 개선 (PM2 cron) vs do-nothing
