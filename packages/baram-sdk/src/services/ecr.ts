...
import type { BaramConfig, ECRData, TierLevel } from '../types';
import { TEE_TYPES, TeeType, TIER_NAMES } from '../types';
 ...
 */
export async function fetchECRByRequestId(
  client: SuiClient,
  config: BaramConfig,
  requestId: number,
): Promise<ECRData | null> {
  if (!config.compliance.packageId) {
    return null;
  }
  ...
  try {
    const events = await client.queryEvents({
      query: {
        MoveEventType: `${config.compliance.packageId}::compliance::ComplianceRecordCreated`,
      },
      limit: 50,
      order: 'descending',
    });
