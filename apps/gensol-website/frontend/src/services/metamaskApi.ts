 ...
 */
export async function requestChallenge(walletAddress: string): Promise<MetaMaskChallengeResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new MetaMaskApiError(data.message || 'Failed to request challenge', response.status, data)
    }
    ...
    throw new MetaMaskApiError(error instanceof Error ? error.message : 'Network error occurred')
  }
}
...
export async function verifySignature(
  ...
  try {
    ...
    if (!response.ok) {
      throw new MetaMaskApiError(data.message || 'Failed to verify signature', response.status, data)
    }
...
export async function authenticateWithMetaMask(
  ...
  // Step 3: Verify 요청
  const verifyResponse = await verifySignature(walletAddress, signature, challengeResponse.nonce)
