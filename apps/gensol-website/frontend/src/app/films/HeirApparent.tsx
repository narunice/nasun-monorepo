...
import HeirApparentCharacter from "@/assets/images/Firefly_Flux_A stylized illustration of a young Asian woman wearing a more futuristic sci-fi outfi 54988.webp"
import BlueBkgd from "@/assets/images/Blue-bkgd.webp"

const HeirApparentSection = () => {
  return (
    <>
      {/* The Heir Apparent 섹션 데스크탑 (lg 이상) */}
      <section className="hidden lg:flex relative w-full flex-row h-[calc(100vh-64px)]">
        ...
        <div className="w-1/2 h-full relative overflow-visible z-10">
          <FadeIn>
            ...
            ></div>
            <div
              className="absolute w-auto h-[95%] inset-x-0 bottom-0 flex bg-no-repeat"
              style={{
                backgroundImage: `url(${HeirApparentCharacter})`,
          ...
          <div>
            <FadeInUp>
              <div className="w-fit mb-6">
                <h6>{filmsContent.heirApparent.category}</h6>
                <h3 className="!font-light">{filmsContent.heirApparent.title}</h3>
              </div>
            </FadeInUp>

            <FadeInUp>
              <div className="space-y-6 max-w-[554px]">
                {filmsContent.heirApparent.paragraphs.map((paragraph, index) => (
            ...
            <FadeInUp>
              <div className="mt-12 max-w-[554px]">
                <span className="font-normal">{filmsContent.heirApparent.status.label}</span>
                <p>{filmsContent.heirApparent.status.description}</p>
              </div>
            </FadeInUp>
          </div>
        </div>
      ...
      {/* The Heir Apparent 섹션 모바일 (lg 미만) */}
...
export default HeirApparentSection
