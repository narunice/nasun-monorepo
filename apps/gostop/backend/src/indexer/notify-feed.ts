...
export async function notifyFeed(sql: Sql, payload: FeedNotifyPayload): Promise<void> {
  ...
  // Suppress NOTIFY for historical events during indexer catch-up. Window
  // matches hub.replay() cutoff so the live stream and replay-on-connect
  // agree on what counts as "live".
  if (Date.now() - payload.ts > env.feed.liveWindowMs) return;
  let json: string;
  try {
