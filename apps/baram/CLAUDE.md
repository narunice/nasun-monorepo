...
| 함수 | 호출자 | 설명 |
|------|--------|------|
| `create_record` | Admin | ECR 생성 (더 이상 호출되지 않음) |
...
| `is_delegated` / `is_tee_verified` / `has_delegation` / `is_chained` | View | 파생 조건 확인 |
...
### Compliance Registry (FROZEN — 기존 ECR 보존)
...
| [sui-client.ts](executor-nitro/src/host/sui-client.ts) | On-chain settlement + AER 생성 + F-2 PTB Call 3/4 |
...
| [AER_DESIGN.md](docs/AER_DESIGN.md) | AIExecutionReport 구현 레퍼런스 (8카테고리, 31필드, fresh deploy) |
