/**
 * useTraderConfig — load/save a single agent's trader bot config.
 *
 * One config per agent in 2A-1. Returns null while loading or if none exists.
 */
...
export interface UseTraderConfigResult {
  ...
  loading: boolean;
  error: string | null;
  save: (next: Omit<TraderConfig, 'id' | 'walletAddress' | 'createdAt' | 'updatedAt'>) => Promise<TraderConfig | null>;
  remove: () => Promise<void>;
  ...
  const save = useCallback<UseTraderConfigResult['save']>(
    async (next) => {
      ...
      const merged: TraderConfig = {
        id: agentAddress, // 1 config per agent in 2A-1
        walletAddress,
