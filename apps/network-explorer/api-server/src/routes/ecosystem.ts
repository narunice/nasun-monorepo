...
// Box-first bulk profile read (box POST /profile/batch when IDENTITY_READ_MODE=flip, DynamoDB
// fallback) lives in ../lib/profile-batch-read.ts so the weekly settlement scripts share one path.
// batchGetProfiles returns the raw 15-field UserProfiles item shape; the 3-pass primary-override +
// linked-secondary enrich below is unchanged.
