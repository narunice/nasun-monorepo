...
// AWS-exit DAL S3.R4: the box nasun-identity mirror can serve the same UserProfiles rows the
// leaderboard resolves (display-only -> staleness is cosmetic and self-heals on the next dal-reload).
// IDENTITY_READ_MODE=flip routes batchGetProfiles' reads through the box POST /profile/batch with a
// DynamoDB fallback on any box error/timeout/misconfig. The box returns the SAME per-identity raw
// item shape as a UserProfiles BatchGet (the 15-field PROFILE_PROJECTION), so the 3-pass
// primary-override + linked-secondary enrich below is unchanged. Reached over the public nginx route
// (no WireGuard from node-3).
...
const PROFILE_EAN = { '#pr': 'provider', '#em': 'email', '#tgm': 'isTelegramMember', '#rl': 'role' };

// Read every box-read knob at call time (not import time) so a runtime .env edit + pm2 restart, or a
// hot rollback/repoint, takes effect uniformly -- avoids the footgun where MODE is hot but URL/SECRET
// are frozen. enabled requires flip + both URL and SECRET (fail-closed -> DynamoDB).
function identityReadConfig() {
...
// Box-served bulk profile read. Returns the per-identity raw item map, or null when not flipped or
// on any failure so the caller falls back to DynamoDB. Display-only, so a failed chunk fails the
// whole call (not per-chunk) -> a single batchGetProfiles result never mixes box and DynamoDB rows
// (mixing across the 3 passes of one fetchProfilesBatch is still possible and harmless: see Pass 2/3).
async function boxProfileBatch(ids: string[]): Promise<Map<string, Record<string, unknown>> | null> {
  ...
  try {
    ...
    console.warn('[leaderboard] box /profile/batch failed, falling back to DynamoDB:', err instanceof Error ? err.message : err);
    return null;
  }
}
...
async function batchGetProfiles(ids: string[]): Promise<Map<string, Record<string, unknown>>> {
...
async function fetchProfilesBatch(identityIds: string[]): Promise<Map<string, ProfileCacheEntry>> {
  ...
  if (missing.length > 0) {
    try {
      // Pass 1: fetch the missing identities (box /profile/batch when flipped, else DynamoDB).
      const fetched = await batchGetProfiles(missing);
      ...
      const primaries = await batchGetProfiles(primaryIds);
      ...
      const secondaries = await batchGetProfiles(secondaryIds);
