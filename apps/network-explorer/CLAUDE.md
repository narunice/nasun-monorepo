...
> **Why colocate api-server with sui-indexer on node-3**: scanner들이 sui_indexer DB(PostgreSQL)와 fullnode RPC를 동시에 읽으므로 same-host colocation으로 latency가 사라지고 네트워크 비용도 0. prod EC2(43.200.67.52)는 nasun.io + pado.finance + chat-server 트래픽을 이미 감당 중이라 부적합. 자세한 인프라는 [../../docs/infrastructure.md](../../docs/infrastructure.md) 참조.
