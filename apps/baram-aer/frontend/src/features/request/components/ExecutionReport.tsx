/**
 * ExecutionReport - Modal displaying AI Execution Report
 */
...
export function ExecutionReport({ requestId, metadata, onClose }: ExecutionReportProps) {
  const { aer, isLoading, error } = useAER(requestId);
  ...
  const hasLocalData = !aer && metadata && (metadata.resultHash || metadata.teeType);
        ...
        <div className="px-5 pb-5">
          {isLoading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-2 text-sm text-[var(--color-text-muted)]">Loading report...</span>
            </div>
          )}

          {error && (
            <div className="py-8 text-center">
              <p className="text-sm text-[var(--color-error)]">Failed to load report</p>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">{error}</p>
            </div>
          )}

          {!isLoading && !error && !aer && !hasLocalData && (
            <div className="py-8 text-center">
              ...
              <p className="text-sm text-[var(--color-text-muted)]">Report not yet available</p>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">
          ...
          {aer && (
            <OnChainReceiptContent aer={aer} onClose={onClose} />
          )}
        </div>
      </div>
      </div>
