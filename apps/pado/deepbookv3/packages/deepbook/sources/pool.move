...
/// Tier-aware variant of `place_limit_order`. Reads sender's tier from the
/// shared `nasun_tier::tier::TierRegistry` and discounts taker/maker baseline
/// fees by `policy::fee_discount_bps(tier)`. The v1 entry above remains the
/// baseline path for bots / non-tier-aware callers.
public fun place_limit_order_v2<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    balance_manager: &mut BalanceManager,
    trade_proof: &TradeProof,
    tier_registry: &nasun_tier::tier::TierRegistry,
    client_order_id: u64,
    order_type: u8,
    self_matching_option: u8,
    price: u64,
    ...
    self.place_order_int_with_tier(
        balance_manager,
        trade_proof,
        tier_registry,
        client_order_id,
...
/// Tier-aware variant of `place_market_order`.
...
/// Tier-aware variant of `swap_exact_base_for_quote`.
public fun swap_exact_base_for_quote_v2<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    tier_registry: &nasun_tier::tier::TierRegistry,
    base_in: Coin<BaseAsset>,
    deep_in: Coin<DEEP>,
...
/// Tier-aware variant of `swap_exact_quote_for_base`.
public fun swap_exact_quote_for_base_v2<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    tier_registry: &nasun_tier::tier::TierRegistry,
    quote_in: Coin<QuoteAsset>,
    deep_in: Coin<DEEP>,
...
/// Tier-aware variant of `swap_exact_quantity`. Mirrors v1 logic but routes
/// through `place_market_order_v2` so that the temporary BalanceManager's
/// fees are computed using the sender's tier.
public fun swap_exact_quantity_v2<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    tier_registry: &nasun_tier::tier::TierRegistry,
    base_in: Coin<BaseAsset>,
    quote_in: Coin<QuoteAsset>,
...
/// Set baseline trade params (taker_fee / maker_fee / stake_required) via
/// admin path, bypassing the proposal/vote flow. Required to lower the
/// per-pool baseline fee to Hyperliquid retail level so that NSI tier
/// discount produces meaningful effective rates (tier 3 ≈ Hyperliquid VIP).
/// Whitelisted pools cannot change.
public fun admin_set_trade_params<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    _cap: &DeepbookAdminCap,
    taker_fee: u64,
    maker_fee: u64,
    ...
    inner
        .state
        .governance_mut(ctx)
        .set_trade_params_by_admin(taker_fee, maker_fee, stake_required, ctx);
}
...
// === Private Functions ===
/// Tier-aware mirror of `place_order_int`. Identical control flow, except
/// the underlying State call is `process_create_with_tier` which applies
/// the sender's NSI tier discount to taker/maker fees.
fun place_order_int_with_tier<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    balance_manager: &mut BalanceManager,
    trade_proof: &TradeProof,
    tier_registry: &nasun_tier::tier::TierRegistry,
