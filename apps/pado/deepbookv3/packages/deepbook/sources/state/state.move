...
/// Tier-aware variant of `process_create`. Applies `nasun_tier::policy::
/// fee_discount_bps` to the taker baseline fee BEFORE EWMA penalty is added,
/// and to the maker fee by the same factor. The EWMA penalty itself stays
/// undiscounted (anti-abuse surcharge, orthogonal to tier reward).
///
/// Plain integer arithmetic — `math::mul` is fixed-point (FLOAT_SCALING=1e9)
/// and would zero out for any sub-FLOAT_SCALING fee value.
public(package) fun process_create_with_tier(
    self: &mut State,
    order_info: &mut OrderInfo,
    ewma_state: &EWMAState,
    pool_id: ID,
    ...
    let taker_fee_baseline = self
        .governance
        .trade_params()
        .taker_fee_for_user(account_stake, account_volume_in_deep);
    let maker_fee_baseline = self.governance.trade_params().maker_fee();

    let view = nasun_tier::tier::get(tier_registry, ctx.sender());
    let t = nasun_tier::tier::tier_of(&view);
    let discount_bps = nasun_tier::policy::fee_discount_bps(t);
    let taker_fee_discounted =
        (taker_fee_baseline * (10000 - discount_bps)) / 10000;
    let maker_fee = (maker_fee_baseline * (10000 - discount_bps)) / 10000;

    let taker_fee = ewma_state.apply_taker_penalty(taker_fee_discounted, ctx);
    if (taker_fee > taker_fee_discounted) {
        event::emit(TakerFeePenaltyApplied {
            pool_id,
            balance_manager_id: order_info.balance_manager_id(),
            order_id: order_info.order_id(),
            taker_fee_without_penalty: taker_fee_discounted,
            taker_fee,
        });
    };
