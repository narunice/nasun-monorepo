...
> 참고: contracts-nsa는 이전 체인 ID (`56c8b101`)에서 배포됨. V6 마이그레이션 필요.
...
2. **Oracle 미배포**: DevOracle이 V6에 재배포되지 않아 `VITE_ORACLE_*` 변수가 비어있음
3. **Trading Pools 미생성**: `VITE_POOL_NBTC_NUSDC`, `VITE_POOL_NASUN_NUSDC`가 비어있음
4. **contracts-nsa 마이그레이션**: 이전 체인 ID (`56c8b101`)에서 배포됨, V6 재배포 필요
5. **Staging Chain ID 차이**: staging은 `56c8b101`, development은 `12bf3808` 사용
