...
| Phase 18 | ✅ 완료 | Prototype Polish | 트레이딩 UX 개선, 키보드 단축키, 수수료 프리뷰 |
| Phase 19 | ✅ 완료 | Social Layer | Global Chat, Leaderboard, Trader Profiles, Trading Competitions |
...
| Phase 21 | 📋 계획 | Perp/Margin/Lending Frontend | 컨트랙트 V7 배포 완료, 프론트엔드 .env 연동 필요 |
...
| 항목 | 값 |
|------|-----|
| Package | `packages/devnet-config/devnet-ids.json` → `oracle` 참조 |
| Status | ✅ V7 배포 완료 |

**Unified Margin (unified_margin)**:
| 항목 | 값 |
|------|-----|
| Package | `packages/devnet-config/devnet-ids.json` → `margin` 참조 |
| Status | ✅ V7 배포 완료 |

**Perpetuals DEX (pado_perp)**:
| 항목 | 값 |
|------|-----|
| Package | `packages/devnet-config/devnet-ids.json` → `perp` 참조 |
| Status | ✅ V7 배포 완료 |

**Lending (pado_lending)**:
| 항목 | 값 |
|------|-----|
| Package | `packages/devnet-config/devnet-ids.json` → `lending` 참조 |
| Status | ✅ V7 배포 완료 |
...
├── bots/                            # 자동화 봇 (price-updater, liquidation-keeper, lp-bot)
├── chat-server/                     # WebSocket + HTTP 서버 (Chat, Leaderboard, Competitions)
...
| `/leaderboard` | LeaderboardPage | 트레이딩 볼륨 순위 |
| `/leaderboard/trader/:address` | TraderProfilePage | 트레이더 상세 (통계, 체결 이력) |
...
### social/ - Global Chat (Phase 19)

실시간 채팅 (WebSocket), 닉네임 설정, 트레이딩 페이지 연동.

**Components**: ChatPanel, ChatMessage, ChatInput, ChatToggleButton, FloatingChatPopup, MobileChatDrawer, SetNicknameModal

**Hooks**: useChat, useChatPanel, useFloatingPanel

### leaderboard/ - Trading Volume Rankings (Phase 19)

DeepBook OrderFilled 이벤트 기반 트레이딩 볼륨 순위.

**Components**: LeaderboardTable, TraderRow, RankBadge, PeriodSelector, MyRankCard, TraderProfileHeader, TraderFillsTable

**Hooks**: useLeaderboard, useTraderStats, useTraderFills
...
| LP Bot | `bots/lp-bot.ts` | Grid market making (Binance 가격 기준), atomic cancel+place, arbitrage, auto faucet refill (10초 간격) |
...
WebSocket + HTTP 서버. Global Chat, Leaderboard Indexer, Competition API 통합.

| 기능 | 설명 |
|------|------|
| Global Chat | WebSocket 실시간 채팅, signature-based 인증, 닉네임, SQLite 저장 |
| Leaderboard Indexer | DeepBook OrderFilled 이벤트 폴링 → SQLite 집계 |
| Competition API | Admin CRUD, 시간 제한 대회, Bearer token 인증 |

**REST API**:
| 엔드포인트 | 설명 |
|-----------|------|
| `GET /api/leaderboard` | 기간별 순위 (24h, 7d, 30d, all) |
| `GET /api/leaderboard/trader/:address` | 트레이더 개인 통계 + 체결 이력 |
| `GET /api/competitions` | 대회 목록/상세 |
...
3. **Perp/Margin/Lending 프론트엔드 활성화 대기**: 컨트랙트 V7 배포 완료, `.env` 주소 연동 필요
