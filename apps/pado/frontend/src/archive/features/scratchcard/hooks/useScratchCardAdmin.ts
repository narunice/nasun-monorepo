...
export interface UseScratchCardAdminResult {
  isAdmin: boolean;
  isLoading: boolean;
...
export function useScratchCardAdmin(): UseScratchCardAdminResult {
  ...
  const signAndExecute = useCallback(
    async (tx: Transaction): Promise<AdminResult> => {
      ...
      try {
        ...
        queryClient.invalidateQueries({ queryKey: ['scratchcard-pool'] });
