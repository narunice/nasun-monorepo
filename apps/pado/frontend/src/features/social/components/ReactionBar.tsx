...
function ReactionPicker({ onSelect, onClose }: { onSelect: (code: string) => void; onClose: () => void }) {
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-[env(safe-area-inset-bottom)] bg-theme-bg-secondary border-t border-theme-border z-50 sm:absolute sm:bottom-auto sm:left-auto sm:right-auto sm:p-2 sm:rounded-lg sm:border sm:shadow-lg sm:w-auto sm:-top-12">
        <div className="flex items-center justify-center gap-1 sm:gap-0.5">
          {REACTION_CODES.map((code) => (
            <button
              ...
              className="min-w-[44px] min-h-[44px] sm:min-w-[36px] sm:min-h-[36px] flex items-center justify-center text-lg hover:bg-theme-bg-tertiary rounded-lg transition-colors"
            >
              {REACTION_EMOJI[code]}
            </button>
          ))}
