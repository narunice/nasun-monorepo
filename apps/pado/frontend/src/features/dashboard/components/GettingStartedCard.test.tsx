...
describe('GettingStartedCard', () => {
  ...
  // ========================================
  // Step completion detection (3 steps: wallet, faucet, lottery)
  // ========================================
  describe('step completion', () => {
    ...
    it('shows all complete when wallet + balance + lottery done', () => {
      vi.useFakeTimers();
      mockWalletStatus = 'unlocked';
      mockBalance = { totalBalance: '1000000000', formattedBalance: '1.0' };
      localStorage.setItem(LOTTERY_PURCHASED_KEY, String(Date.now()));
  ...
  describe('action buttons', () => {
    it('does not show "Go to Lottery" when wallet not connected', () => {
      renderCard();
      expect(screen.queryByText(/Go to Lottery/)).not.toBeInTheDocument();
    });

    it('does not show "Go to Lottery" when wallet connected but no balance', () => {
      mockWalletStatus = 'unlocked';
      mockBalance = undefined;
      renderCard();
      expect(screen.queryByText(/Go to Lottery/)).not.toBeInTheDocument();
  ...
  describe('progress bar', () => {
    ...
    it('shows 33.33% width when 1 step complete', () => {
      mockWalletStatus = 'unlocked';
      renderCard();
      const bar = document.querySelector('.bg-green-500.rounded-full') as HTMLElement;
      // 1/3 * 100 = 33.33333...%
      expect(parseFloat(bar.style.width)).toBeCloseTo(33.33, 0);
    });

    it('shows 66.67% width when 2 steps complete', () => {
      mockWalletStatus = 'unlocked';
      mockBalance = { totalBalance: '1000000000', formattedBalance: '1.0' };
      renderCard();
      const bar = document.querySelector('.bg-green-500.rounded-full') as HTMLElement;
      // 2/3 * 100 = 66.66666...%
      expect(parseFloat(bar.style.width)).toBeCloseTo(66.67, 0);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('updates lottery step reactively when LOTTERY_PURCHASE_EVENT fires', async () => {
      mockWalletStatus = 'unlocked';
      mockBalance = { totalBalance: '1000000000', formattedBalance: '1.0' };
      renderCard();
      ...
      // Simulate lottery purchase event
      const { act } = await import('@testing-library/react');
      act(() => {
        localStorage.setItem(LOTTERY_PURCHASED_KEY, String(Date.now()));
        document.dispatchEvent(new CustomEvent('pado:lottery-purchased'));
      });

      // After event: all steps complete -> card shows completion message
  ...
  describe('passkey wallet', () => {
    ...
    it('shows "Go to Lottery" for passkey user with balance', () => {
      mockIsPasskeyUnlocked = true;
      mockBalance = { totalBalance: '1000000000', formattedBalance: '1.0' };
      renderCard();
      const link = screen.getByText(/Go to Lottery/);
