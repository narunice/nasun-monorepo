/**
 * FIFA World Cup 2026 prediction-market batch creator (TheSportsDB).
 *
 * Emits TWO binary markets per fixture:
 *   home_win          -> "Will <home> beat <away>?" (draw resolves NO)
 *   total_score_over  -> "Will <home> vs <away> have over <line> goals?"
 *
 * The Over/Under line balances strong-team group games (Brazil/Germany/Mexico
 * win markets would otherwise skew YES); the home_win line carries the toss-up
 * fixtures (Korea/USA). Both resolve from the same TheSportsDB lookupevent the
 * keeper already polls, free tier key=3.
 *
 * EventIds + kickoff UTC verified 2026-06-01 against
 * https://www.thesportsdb.com/api/v1/json/3/lookupevent.php?id=<id>
 * (strTimestamp is UTC; cross-checked vs published local kickoffs).
 *
 * Required env:
 ...
 *        apps/pado/bots/scripts/create-worldcup-batch-2026-06.ts --dry-run
 */
...
// 5 fixtures, one home_win market each (user-selected default).
// To also emit Over/Under markets, add 'total_score_over' + an overLine.
const FIXTURES: Fixture[] = [
  {
    eventId: '2391728',
    homeTeam: 'Mexico',
    awayTeam: 'South Africa',
    kickoffUtc: '2026-06-11 19:00:00 UTC',
    kinds: ['home_win'],
  },
  {
    eventId: '2461103',
    ...
    kickoffUtc: '2026-06-12 02:00:00 UTC',
    kinds: ['home_win'],
  },
  {
    eventId: '2391729',
    homeTeam: 'USA',
    awayTeam: 'Paraguay',
    kickoffUtc: '2026-06-13 01:00:00 UTC',
    kinds: ['home_win'],
  },
  {
    eventId: '2391730',
    homeTeam: 'Brazil',
    awayTeam: 'Morocco',
    kickoffUtc: '2026-06-13 22:00:00 UTC',
    kinds: ['home_win'],
  },
  {
    eventId: '2391733',
    homeTeam: 'Germany',
    awayTeam: 'Curacao',
    kickoffUtc: '2026-06-14 17:00:00 UTC',
    kinds: ['home_win'],
  },
];
...
async function main(): Promise<void> {
  ...
  if (FIXTURES.length === 0) {
    console.error('FIXTURES is empty.');
    process.exit(1);
  }
