/**
 * Custom batch (2026-05-12):
 *   Crypto + US stock prediction markets closing 3, 4, and 5 days out.
 *   Mirrors create-custom-batch-2026-05-09.ts on-chain signature so the
 *   prediction-keeper auto-discovers them.
 *
 *   Design intent: prior batches skewed toward "trivially YES" outcomes
 *   because biasPct was small (+/- 0.3 to 2.0 %) over a 3-5 day horizon.
 *   This batch picks thresholds large enough that the outcome genuinely
 *   depends on a directional move, and alternates sign so neither YES
 *   nor NO is the default position across the book.
 *
 *   Heuristic targets (over 3-5 trading days):
 *     - Crypto biasPct ~ +/- 3 to 6 % (roughly within 1 sigma of weekly vol).
 *     - Stocks biasPct ~ +/- 2 to 3.5 % (lower realized vol than crypto).
 *
 * Usage:
...
// biasPct sign convention:
//   positive => threshold above live price (NO is the "no-move" default,
//               YES requires the asset to rally past the threshold)
//   negative => threshold below live price (YES is the "no-move" default,
//               NO requires the asset to break down past the threshold)
// Sign is alternated across the book so the YES side is not uniformly favored.
const SPECS: Spec[] = [
  // --- 3-day markets ---
  { kind: 'crypto', symbol: 'BTC',  binanceSymbol: 'BTCUSDT',  displayName: 'Bitcoin',  decimals: 0, daysOut: 3, biasPct: +3.5, label: 'BTC 3d up' },
  { kind: 'crypto', symbol: 'ETH',  binanceSymbol: 'ETHUSDT',  displayName: 'Ethereum', decimals: 2, daysOut: 3, biasPct: -4.0, label: 'ETH 3d down' },
  { kind: 'stock',  ticker: 'NVDA', market: 'NYSE', currency: 'USD', displayName: 'NVIDIA Corporation', daysOut: 3, biasPct: +2.5, label: 'NVDA 3d-close up' },

  // --- 4-day markets ---
  { kind: 'crypto', symbol: 'SOL',  binanceSymbol: 'SOLUSDT',  displayName: 'Solana',   decimals: 2, daysOut: 4, biasPct: +5.0, label: 'SOL 4d up' },
  { kind: 'crypto', symbol: 'XRP',  binanceSymbol: 'XRPUSDT',  displayName: 'XRP',      decimals: 4, daysOut: 4, biasPct: -4.0, label: 'XRP 4d down' },
  { kind: 'stock',  ticker: 'TSLA', market: 'NYSE', currency: 'USD', displayName: 'Tesla, Inc.',       daysOut: 4, biasPct: -3.0, label: 'TSLA 4d-close down' },

  // --- 5-day markets ---
  { kind: 'crypto', symbol: 'DOGE', binanceSymbol: 'DOGEUSDT', displayName: 'Dogecoin', decimals: 5, daysOut: 5, biasPct: +6.0, label: 'DOGE 5d up' },
  { kind: 'crypto', symbol: 'BNB',  binanceSymbol: 'BNBUSDT',  displayName: 'BNB',      decimals: 2, daysOut: 5, biasPct: -3.5, label: 'BNB 5d down' },
  { kind: 'stock',  ticker: 'AAPL', market: 'NYSE', currency: 'USD', displayName: 'Apple Inc.',        daysOut: 5, biasPct: +2.0, label: 'AAPL 5d-close up' },
  { kind: 'stock',  ticker: 'MSFT', market: 'NYSE', currency: 'USD', displayName: 'Microsoft Corp.',   daysOut: 5, biasPct: -2.0, label: 'MSFT 5d-close down' },
];
