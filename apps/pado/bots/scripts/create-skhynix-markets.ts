/**
 * Create SK Hynix (000660.KS) Prediction Markets
 *
 * One-off batch: 4 daily-close markets at 3d / 5d / 7d / 9d horizons.
 * Resolution: Yahoo Finance (Twelve Data free tier does not list KRX).
 *
 * Strike selection (2026-05-12 close = 1,835,000 KRW):
 *   - 3d > 1,900,000   momentum continuation (+3.5%)
 *   - 5d < 1,800,000   mild pullback        (-1.9%)
 *   - 7d > 1,950,000   52w-high breakout    (+6.3%)
 *   - 9d < 1,750,000   meaningful retrace   (-4.6%)
 *
 * Directions mixed (2 above / 2 below) and strikes set within ~1 std-dev of
 * current price so neither YES nor NO is the obvious side.
 *
 * Usage:
...
const FINANCE_TEMPLATES: FinanceTemplate[] = [
  {
    ...
    horizons: [
      { label: '3d', daysFromNow: 3, threshold: 1_900_000, op: '>' },
      { label: '5d', daysFromNow: 5, threshold: 1_800_000, op: '<' },
      { label: '7d', daysFromNow: 7, threshold: 1_950_000, op: '>' },
      { label: '9d', daysFromNow: 9, threshold: 1_750_000, op: '<' },
    ],
  },
];
...
function lintHorizon(t: FinanceTemplate, h: Horizon): void {
  if (h.op !== '>' && h.op !== '<') {
    throw new Error(
      `lint: ${t.ticker} ${h.label}: op must be '>' or '<' (got ${h.op})`,
    );
  }
  if (!Number.isFinite(h.threshold) || h.threshold <= 0) {
    throw new Error(`lint: ${t.ticker} ${h.label}: threshold must be positive`);
  }
  if (t.currency === 'KRW' && !Number.isInteger(h.threshold)) {
    throw new Error(`lint: ${t.ticker} ${h.label}: KRW threshold must be integer`);
  }
}
...
async function main(): Promise<void> {
  ...
  if (dryRun) {
    console.log(`[DRY RUN] Would create ${specs.length} SK Hynix markets:\n`);
  ...
  console.log(`Creating ${specs.length} SK Hynix markets`);
