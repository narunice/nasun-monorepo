/**
 * Mixed prediction-market batch (2026-05-22).
 *
 * 14 binary markets across space (LL2), sports (TheSportsDB), weather
 * (Open-Meteo), crypto (Binance), and stocks (Twelve Data + Yahoo).
 *
 ...
 *
 * Required env: PREDICTION_ADMIN_KEY, PREDICTION_RESOLVER_KEY,
 *               PREDICTION_PACKAGE_ID, PREDICTION_ADMIN_CAP (optional default).
 *
 * Usage:
 *   node --env-file=.env --import tsx scripts/create-mixed-batch-2026-05-22.ts --dry-run
 *   node --env-file=.env --import tsx scripts/create-mixed-batch-2026-05-22.ts
 */
...
interface SpaceSpec {
  ...
  missionName: string;
  netUtc: string;            // "YYYY-MM-DD HH:MM:SS UTC"
  pad: string;
...
interface SportsSpec {
  ...
  threshold?: number;        // required when field=total_score_over
}
...
interface CryptoSpec {
  ...
  decimals: number;
  threshold: number;
  closeUtc: string;          // "YYYY-MM-DD HH:MM:SS UTC"
}

interface StockSpec {
  kind: 'stock';
  ticker: string;            // Twelve Data symbol (e.g. NVDA, 005930.KS)
  market: 'US' | 'KRX';
  currency: 'USD' | 'KRW';
  displayName: string;
  threshold: number;
  closeUtc: string;          // session close in UTC
}

type Spec = SpaceSpec | SportsSpec | WeatherSpec | CryptoSpec | StockSpec;

const SPECS: Spec[] = [
  // ===== Space (5) =====
  {
    kind: 'space',
    launchId: 'ed83366c-872c-4484-97c1-bc74832304fc',
    missionName: 'Starship Flight 12',
    netUtc: '2026-05-22 22:30:00 UTC',
    pad: 'Orbital Launch Pad 2',
    field: 'mission_success',
  },
  {
    kind: 'space',
    launchId: 'a0e0f590-1cb4-4b6b-845c-0087633af132',
    missionName: 'Shenzhou 23',
    netUtc: '2026-05-24 15:08:00 UTC',
    pad: 'Launch Area 91 (SLS-1 / 921), Jiuquan',
    field: 'mission_success',
  },
  {
    kind: 'space',
    launchId: '9e27a1ed-e81a-4918-b32b-0380cf3a9f8f',
    missionName: 'South Korean ADD Solid-Fuel SLV — Demo Flight',
    netUtc: '2026-06-01 05:00:00 UTC',
    pad: 'ADD Offshore launch platform',
    field: 'on_schedule_24h',
  },
  {
    kind: 'space',
    launchId: 'e1a3b702-464f-435d-8c1f-ade9be196b77',
    missionName: 'H3-30 Test Flight',
    netUtc: '2026-06-10 00:53:59 UTC',
    pad: 'Yoshinobu Launch Complex LP-2',
    field: 'mission_success',
  },
  {
    kind: 'space',
    launchId: 'ff0a6a32-6513-4e85-9fd8-a2d022add74d',
    missionName: 'Atlas V 551 — Amazon Leo (LA-07)',
    netUtc: '2026-05-29 23:33:00 UTC',
    pad: 'Space Launch Complex 41, Cape Canaveral',
    field: 'on_schedule_24h',
  },
  ...
  // ===== Crypto (2) =====
  {
    ...
    decimals: 0,
    threshold: 85000,
    closeUtc: '2026-06-30 23:59:00 UTC',
  },
  {
    kind: 'crypto',
    symbol: 'ETH',
  ...
  // ===== Stocks (2) =====
  {
    ...
    displayName: 'NVIDIA',
    threshold: 230,
    closeUtc: '2026-05-30 20:00:00 UTC',   // US session close ~20:00 UTC
  },
  {
    kind: 'stock',
    ticker: '005930.KS',
    ...
    threshold: 300000,
    closeUtc: '2026-05-30 06:30:00 UTC',   // KRX close 15:30 KST = 06:30 UTC
  },
];
...
function loadActiveSuiKeystoreKeypair(): Ed25519Keypair {
  // Read ~/.sui/sui_config/client.yaml active_address, then find the matching
  // private key in ~/.sui/sui_config/sui.keystore. Mirrors how `sui client` resolves
  // the signer — keeps the create_market admin in sync with `sui client active-address`.
  ...
  for (const k of keys) {
    // sui.keystore entries are base64 of (flag || privkey). flag 0x00 = ed25519.
    const raw = Buffer.from(k, 'base64');
    if (raw[0] !== 0x00) continue; // skip secp256k1/secp256r1
...
function buildSportsMarket(spec: SportsSpec): Market {
  ...
  if (spec.field === 'home_win') {
    ...
    return {
      ...
      description:
        `Binary outcome on the regulation/full-time score of ${spec.homeTeam} vs ${spec.awayTeam} ` +
        `(kickoff ${spec.kickoffUtc}, ${spec.league}). Resolves YES iff ${spec.homeTeam}'s ` +
        `final score is strictly greater than ${spec.awayTeam}'s. A draw or away win resolves NO. ` +
        `If postponed past the resolve deadline the market is auto-cancelled.`,
  ...
  return {
    spec, category: 'sports',
    question: `⚽ ${spec.league} — Will ${spec.homeTeam} vs ${spec.awayTeam} have more than ${thr} total goals?`,
    description:
      `Binary outcome on the regulation/full-time total goals of ${spec.homeTeam} vs ${spec.awayTeam} ` +
      `(kickoff ${spec.kickoffUtc}, ${spec.league}). Resolves YES iff home_score + away_score > ${thr}. ` +
...
function buildStockMarket(spec: StockSpec): Market {
  const closeMs = parseUtc(spec.closeUtc);
  // Stocks: deadline 3 days after close so weekend/holiday gaps are absorbed.
...
function buildMarket(spec: Spec): Market {
  switch (spec.kind) {
    ...
    case 'sports':  return buildSportsMarket(spec);
...
function specLabel(spec: Spec): string {
  switch (spec.kind) {
    ...
    case 'sports': return `[sports] ${spec.homeTeam} vs ${spec.awayTeam} / ${spec.field}`;
...
async function main(): Promise<void> {
  ...
  // Admin: prefer env PREDICTION_ADMIN_KEY; fall back to local sui keystore active address.
  // Resolver: REQUIRES PREDICTION_RESOLVER_ADDRESS (no key derivation — script is used for
  // off-prod creation against the already-deployed prediction-keeper resolver address).
