...
- Claude는 프롬프트에 대해 항상 **한국어로 답변**해주세요.
- 문서로 저장해달라는 프롬프트를 받으면 별도의 주문이 없는 이상 항상 `<MONOREPO>/nasun-website/doc` 경로에 저장하세요.
...
- ✅ 계정 매핑: Dev (@Naru010110), Prod (@Nasun_io)
...
2. **✅ Phase 4-2: API Keys 발급**:
   - API Key: __TWITTER_API_KEY__
   - API Key Secret: (발급 완료)
   - Bearer Token: (발급 완료)

3. **✅ Phase 4-3: OAuth 2.0 설정**:
   - Client ID: __TWITTER_CLIENT_ID__
   - Client Secret: (발급 완료)
...
5. **✅ Phase 4-5: Secrets Manager 업데이트**:
   ...
   - Version ID: __SECRETS_MANAGER_VERSION_UUID__
...
- Secrets Manager Version: __SECRETS_MANAGER_VERSION_UUID__
...
3. **✅ Phase 3-3: Secrets Manager 시크릿 생성**:
   ...
   - ARN: `arn:aws:secretsmanager:ap-northeast-2:__AWS_PROD_ACCOUNT__:secret:nasun-twitter-tokens-prod-2ekpAS`
...
**배포 횟수**: 2회
**최종 검증**: ✅ 모든 기능 정상 작동 (test_handle, SunyoungP29745 테스트)
...
cd <MONOREPO>/nasun-website/cdk/lambda-src/auth-twitter
...
# ✅ 개발 환경 배포 (타겟: @Naru010110, AWS 계정: __AWS_DEV_ACCOUNT__)
pnpm deploy:dev

# ✅ 프로덕션 환경 배포 (타겟: @Nasun_io, AWS 계정: __AWS_PROD_ACCOUNT__)
pnpm deploy:prod
