...
Rewards stakers for their daily epoch reward accumulation. Triggered from the
`points-scanner.ts` scanLoop on the first iteration after UTC day-rollover (no
minute gate — fires shortly after 00:00 UTC; the 00:05 gate applies only to
`takeDailySnapshot`). `daily-nft-check.ts:fetchIdentityStakeData` reads
`suix_getStakes`'s `estimatedReward` for every staking identity with bounded
concurrency (`RPC_CONCURRENCY = 20`, lowered from 50 on 2026-05-12 to reduce
midnight fullnode memory burst). State is bulk-upserted, then a `staking-reward`
row is inserted per identity with:
...
**Reliability layers (post-2026-05-12)**: transient fullnode 502/503/504 and
timeouts are absorbed by centralized retry+backoff in `rpc.ts` (3 attempts,
exponential backoff with nginx `Retry-After` honored). For longer fullnode
restart windows (~60-90s observed), partial-failure path still applies but
`stakingRetryNeeded` keeps the daily gate open so the next scanLoop iteration
(~60s) re-attempts the missed identities — `ON CONFLICT DO NOTHING` on the
`stkr:` digest guarantees idempotency.
