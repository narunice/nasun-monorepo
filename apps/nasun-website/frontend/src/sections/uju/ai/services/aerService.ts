 ...
 * Queries ExecutionReportCreated events, then loads the full object from chain.
 */

import { suiClient } from '@/lib/sui-client';
import { AER_CONFIG, AER_STATUS_NAMES, TIER_NAMES, type TierLevel } from './network';
...
export async function fetchAERByRequestId(requestId: number): Promise<AERData | null> {
  ...
  try {
    const events = await suiClient.queryEvents({
      query: { MoveEventType: `${AER_CONFIG.typeOrigin}::aer::ExecutionReportCreated` },
      limit: 50,
      order: 'descending',
    });
    ...
    const obj = await suiClient.getObject({ id: recordId, options: { showContent: true } });
