    ...
    <div className={`flex items-start gap-3 px-4 py-3 border-b border-uju-border last:border-0 ${notif.read ? '' : 'bg-white/[0.02]'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className={`text-sm font-light px-2 py-0.5 rounded-md ${badge.bg} ${badge.text}`}>
            {badge.label}
          </span>
          <span className="text-sm text-uju-secondary">{formatRelativeTime(notif.timestamp)}</span>
        </div>
        <p className={`text-sm font-light ${notif.read ? 'text-uju-secondary' : 'text-uju-primary'}`}>
          {notif.title}
        </p>
    ...
    <UjuCard className="p-0 overflow-hidden">
      ...
      {notifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 px-6 text-center gap-3">
          ...
          <p className="text-sm font-light text-uju-secondary">No notifications yet.</p>
