...
export function TotalPointsCard({ bare = false }: TotalPointsCardProps = {}) {
  ...
  // Icon-sized refresh button — kept on the dashboard so users can pull a
  // fresh snapshot without leaving the tab. The activity tab carries a
  // larger refresh control inside the pts-today card; both call into the
  // same useEcosystemScore.refresh().
  const refreshIconButton = (
    <button
      ...
      className="w-7 h-7 flex items-center justify-center rounded-md border border-uju-border/30 text-uju-secondary hover:text-uju-primary hover:border-pado-2/40 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
    >
      <svg
        viewBox="0 0 24 24"
        fill="none"
  ...
  const body = (
    <>
      <UjuSectionHeader accent title="Nasun Points" trailing={refreshIconButton} />
      {isLoading ? (
