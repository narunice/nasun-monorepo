...
export function MarketDetailDark() {
  ...
  return (
    <div className="space-y-6">
      <h5 className="font-semibold text-nasun-white">{t("gtm.headline")}</h5>
        ...
        {(["beta", "sdk", "enterprise", "network"] as const).map((key, i) => (
          <StaggerItem key={key} className="h-full relative">
            <OuterBox
              color="nw3"
              padding="sm"
              className="h-full hover:-translate-y-1 transition-all duration-200 !border !border-br-1/25"
            >
              <span className="text-br-1 font-mono text-xs uppercase tracking-wider block mb-3">
                {t(`gtm.stages.${key}.number`)}
              </span>
              <h6 className="font-semibold text-nasun-white mb-2">
                {t(`gtm.stages.${key}.title`)}
              </h6>
              <p className="text-nasun-white/80">{t(`gtm.stages.${key}.detail`)}</p>
            </OuterBox>
            {i < 3 && (
