...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  "ecosystem-bonus-genesis-pass-airdrop": "bg-violet-500",
  "ecosystem-bonus-alliance-airdrop": "bg-violet-500",
};
...
export const PointsCard: FC<PointsCardProps> = ({ className = "" }) => {
  ...
  if (!hasValidAddress) {
    return (
      <OuterBox color="w2" padding="sm" className={className}>
        <h4 className="text-lg font-semibold uppercase text-nasun-white mb-4 flex items-center gap-2">
          ACTIVITY POINTS{" "}
  ...
  const totalForBar = points.categories.reduce(
  ...
  return (
    <OuterBox color="w2" padding="sm" className={className}>
      ...
      <div className="text-3xl font-bold text-nasun-white mb-3">
        ...
        <span className="text-sm font-normal text-nasun-white/80 ml-2">
          pts
        </span>
      </div>
