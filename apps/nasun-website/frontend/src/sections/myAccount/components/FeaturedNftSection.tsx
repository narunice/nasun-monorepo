...
import { getExplorerNFTUrl, getOpenSeaNFTUrl, type NFTChain } from "@/services/ethereumApi";
    ...
    >
      ...
      <div className="p-3 space-y-1.5">
        <div className="flex items-center justify-between gap-1">
          <p className="text-sm text-gray-200 truncate font-medium flex-1">{displayName}</p>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-500 truncate">{collectionName}</span>
        </div>
      </div>
    </a>
  );
...
export const FeaturedNftSection: FC<FeaturedNftSectionProps> = ({
    ...
    return (
      <div className="flex items-center gap-3 p-4 rounded-lg bg-amber-500/5 border border-amber-500/20 mb-4">
        ...
        <p className="text-sm text-amber-200/80">
          Connect your MetaMask wallet to view your Genesis Pass.
        </p>
      </div>
    );
  }
  ...
  const collectionNameMap = new Map(
    featuredCollections.map((c) => [`${c.contractAddress}:${c.chain}`, c.collectionName])
  );

  return (
    <div className="mb-6">
      <h6 className="font-semibold mb-3 text-amber-200/90">Featured Collection</h6>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {nfts.map((nft, i) => {
          const chain = nft.chain ?? "ethereum";
          const name = collectionNameMap.get(`${nft.contractAddress.toLowerCase()}:${chain}`) || "Featured";
          return (
            <FeaturedNftCard
