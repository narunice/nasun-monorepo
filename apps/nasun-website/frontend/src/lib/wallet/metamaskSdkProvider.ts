 ...
 * On Android, resets timeout when browser becomes visible (tab was suspended).
 */
function withTimeout<T>(
  ...
  const resetTimeout = (reject: (reason: Error) => void) => {
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    ...
    // Android Chrome suspends timers in background tabs.
    // When user returns from wallet app, reset the timeout to give SDK
    // time to reconnect and deliver the response.
    const onVisible = () => {
    ...
    (async () => {
      ...
      if (provider && isMobileBrowser()) {
        provider.on("display_uri", ((...args: unknown[]) => {
          ...
          const isAllowedScheme = /^(ethereum:|https:\/\/metamask\.app\.link\/|https:\/\/link\.metamask\.io\/|metamask:\/\/)/.test(uri);
          if (_activeOperation && uri && !_redirectedForCurrentOp && isAllowedScheme) {
