  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-6 w-full">
        <div className="text-nasun-white/70 text-sm py-8 text-center">Loading dashboard...</div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <OuterBox color="c6" className="w-full !border-nasun-c5/45 !bg-gray-800/50">
          ...
          {!stats?.recentActivity?.length ? (
            <div className="text-nasun-white/70 text-sm">No recent activity.</div>
          ) : (
            <div className="space-y-2">
              {stats.recentActivity.map((activity, index) => (
                <div
                  key={index}
                  className="flex items-start gap-2 text-sm py-2 border-b border-nasun-c5/20 last:border-0"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-nasun-white/60 text-xs">
                  ...
                  {activity.postId && (
                    <button
                      ...
                      className="shrink-0 px-2 py-1 text-xs text-nasun-white/80 hover:text-nasun-white bg-gray-700/70 hover:bg-gray-700 rounded-sm transition-all"
                    >
                      Edit
                    </button>
                  )}
        ...
        <OuterBox color="c6" className="w-full !border-nasun-c5/45 !bg-gray-800/50">
          ...
          {!stats?.topFive?.length ? (
            <div className="text-nasun-white/70 text-sm">
            ...
            <div className="space-y-2">
              {stats.topFive.map((user) => (
                <div
                  key={user.rank}
                  className="flex items-center gap-3 py-2 border-b border-nasun-c5/20 last:border-0"
                >
                  <span className="text-lg font-bold text-nasun-white/80 w-6 text-center">
                    {user.rank === 1
                      ? "🥇"
                      : user.rank === 2
                        ? "🥈"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`text-nasun-white/70 truncate ${user.displayName ? "text-xs" : "text-sm text-nasun-white"}`}>
...
function StatCard({ title, value }: { title: string; value: string }) {
  return (
    <OuterBox color="c6" padding="sm" className="!border-nasun-c5/45 !bg-gray-800/50">
      <div className="text-xs uppercase tracking-widest text-nasun-white/70 mb-1">{title}</div>
