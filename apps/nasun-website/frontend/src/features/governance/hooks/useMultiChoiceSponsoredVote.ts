/**
 * useMultiChoiceSponsoredVote Hook
 *
 * Gas-free voting for multi-choice proposals (Poll type).
 * Same flow as useSponsoredVote but targets multi_choice_proposal module
...
export function useMultiChoiceSponsoredVote() {
  ...
  const vote = async (
    ...
    try {
      ...
      console.error("Multi-choice sponsored vote error:", err);
