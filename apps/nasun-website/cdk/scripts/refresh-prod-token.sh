...
CLIENT_ID="__TWITTER_CLIENT_ID__"
CLIENT_SECRET="__TWITTER_CLIENT_SECRET__"
...
if [[ "$CURRENT_ACCOUNT" != "__AWS_PROD_ACCOUNT__" ]]; then
    echo -e "${RED}❌ AWS 계정 불일치!${NC}"
    echo "   현재: $CURRENT_ACCOUNT"
    echo "   예상: __AWS_PROD_ACCOUNT__ (프로덕션)"
    exit 1
fi
...
echo "   cd <MONOREPO>/nasun-website/cdk"
echo "   ./deploy.sh prod"
echo ""
