/**
 * 🚀 V3 Collect Retweets Handler
 * 
 * X API Basic Plan Rate Limit를 준수하여 트윗의 리트윗 유저를 수집하는 Lambda 함수
...
interface OutputEvent {
  success: boolean;
  batchIndex: number;
  retweetsCollected: EngagementData[];
...
/**
 * 🔧 Collect Retweets V3 Handler
 * 
 * 단일 트윗 배치의 리트윗 유저를 수집합니다.
 * X API Basic Plan의 엄격한 Rate Limit를 준수하여 안전하게 동작합니다.
 * 
 * 핵심 기능:
 * - 배치 내 각 트윗의 리트윗 유저 수집
 * - Rate Limit 헤더 실시간 모니터링
 * - DynamoDB에 리트윗 데이터 저장
 * - CloudWatch 메트릭으로 API 사용량 추적
 * 
 * Rate Limit 관리:
 * - API: /tweets/:id/retweeted_by (5회/15분)
...
export const handler: Handler<InputEvent, OutputEvent> = async (event) => {
  ...
  console.log('🔄 [COLLECT_RETWEETS_V3] 시작:', JSON.stringify(event, null, 2));
  ...
  try {
    ...
    const retweetsCollected: EngagementData[] = [];
    let apiCallsUsed = 0;
    ...
    console.log(`📊 [BATCH_${batchIndex}] ${tweets.length}개 트윗의 리트윗 수집 시작`);
    ...
    for (const tweet of tweets) {
      try {
        console.log(`🔄 [TWEET_${tweet.id}] 리트윗 수집 시작`);


        // 리트윗 유저 수집 (동적 maxResults 적용)
        ...
        if (users && users.length > 0) {
          for (const user of users) {
            const retweetEngagement: EngagementData = {
              tweet_id: tweet.id,
          ...
          console.log(`🔒 [TWEET_${tweet.id}] 수집 완료 마킹 (Passive - Retweets)`);
          ...
          console.log(`🔒 [TWEET_${tweet.id}] 수집 완료 마킹 (Passive - Retweets, 0개)`);
        }
    ...
    const result: OutputEvent = {
      ...
      retweetsCollected: retweetsCollected,
