...
// Routes are matched on path suffix; the EVM Lambda uses the same pattern.
// Path layout (mounted at the API root):
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    console.error('[auth-solana-additional] unhandled:', (err as Error)?.message);
