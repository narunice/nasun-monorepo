 ...
 * `linkedAccounts.solana`, push the FULL resulting sub-object to the box `nasun-identity` service so
 ...
 * leave a `solana` object; a disconnect goes through link-account -> /profile/link-sync instead).
 */
async function mirrorSolanaLink(identityId: string): Promise<void> {
  ...
  try {
    ...
    const account = (fresh.Item as UserProfile | undefined)?.linkedAccounts?.solana;
    if (!account || typeof account !== 'object') return;
    const payload = { identityId, provider: 'solana', account };
    if (authoritative) {
    ...
    console.warn('[mirrorSolanaLink] best-effort mirror failed:', err instanceof Error ? err.message : err);
  }
}
...
export async function findOtherOwnerOfAddress(
  ...
  // AWS-exit DAL read-flip: box-served collision check (/profile/address-owner). box.linked_accounts.
  // solana is LOCKSTEP with DynamoDB because the link write mirrors via authoritativeIdentityWrite
  // (linked-account-merge is in IDENTITY_WRITE_FLIP_ROUTES), so a box 200 -- including
  // ownerIdentityId:null (no collision) -- is authoritative and safely short-circuits the Scan.
  // readProfileFromBox returns null only on unset/non-200/error, which falls through to the
  // authoritative DynamoDB Scan below. Solana addresses are case-sensitive (base58); the box route
  // compares them exactly, mirroring this lambda's addrEq (a === b).
...
export async function appendVerifiedAddress(
  ...
  if (!sol || !sol.walletAddress || sol.manualEntry === true) {
    ...
    await mirrorSolanaLink(identityId);
    return {
      primary: true,
      walletAddress: entry.walletAddress,
      additionalAddresses: [],
