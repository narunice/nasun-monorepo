...
export async function handler(
  ...
  if (!token) {
    console.warn("[airdrop-authorizer] No token in Authorization header");
  ...
  try {
    ...
    if (!identityId) {
      console.warn("[airdrop-authorizer] Token missing sub claim");
      return generatePolicy("anonymous", "Deny", event.methodArn);
    }

    console.log(`[airdrop-authorizer] Authorized: ${identityId}`);
    return generatePolicy(identityId, "Allow", event.methodArn, {
      identityId,
    });
  } catch (error: any) {
    console.warn("[airdrop-authorizer] Token verification failed:", error.message);
    return generatePolicy("anonymous", "Deny", event.methodArn);
  }
}
