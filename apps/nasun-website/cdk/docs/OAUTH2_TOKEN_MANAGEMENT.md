...
|-------------|-------------|-------------|----------------|--------------|
| Development | `nasun-twitter-tokens` | __AWS_DEV_ACCOUNT__ | @Nasun_io (1725466995565752320) | **Disabled** |
| Production | `nasun-twitter-tokens-prod` | __AWS_PROD_ACCOUNT__ | @Nasun_io (1725466995565752320) | Enabled (70 min) |
...
- [ ] **1. Verify current AWS account**
  ...
  # Expected: __AWS_DEV_ACCOUNT__ (dev) or __AWS_PROD_ACCOUNT__ (prod)
  ```
...
- [ ] **3. Verify AWS profile**
  ...
  # Dev: __AWS_DEV_ACCOUNT__
  # Prod: __AWS_PROD_ACCOUNT__ (use --profile nasun-prod)
  ```
...
```bash
# 전체 시스템 점검 (개발 + 프로덕션)
<MONOREPO>/scripts/daily-health-check.sh
```
