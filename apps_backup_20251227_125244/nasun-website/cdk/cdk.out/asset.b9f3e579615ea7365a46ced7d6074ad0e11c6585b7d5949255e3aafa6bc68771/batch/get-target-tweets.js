...
var get_target_tweets_exports = {};
__export(get_target_tweets_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_target_tweets_exports);
...
function getSnapshotDateRange(daysAgo, baseDateStr) {
  ...
  console.log(`\u{1F4C5} [SNAPSHOT] ${daysAgo}\uC77C \uC804 \uB0A0\uC9DC \uBC94\uC704 (\uAE30\uC900: ${baseDate.toISOString()}): ${start.toISOString()} ~ ${end.toISOString()}`);
  return {
...
// src/utils/step-functions-errors.ts
...
var DataValidationError = class extends Error {
  constructor(message, invalidData) {
    super(message);
...
// src/handlers/batch/get-target-tweets.ts
function calculateAdaptiveWaitTime(tweetCount) {
  const MIN_WAIT_SECONDS = 15 * 60;
  ...
  const factor = (20 - tweetCount) / 19;
  const adaptiveWait = Math.round(MIN_WAIT_SECONDS + factor * (MAX_WAIT_SECONDS - MIN_WAIT_SECONDS));
  console.log(`\u{1F4CA} [ADAPTIVE_WAIT] \uD2B8\uC717 ${tweetCount}\uAC1C \u2192 \uB300\uAE30\uC2DC\uAC04 ${Math.round(adaptiveWait / 60)}\uBD84 (${adaptiveWait}\uCD08)`);
  return adaptiveWait;
}
var handler = async (event, context) => {
  const startTime = Date.now();
  console.log("\u{1F680} [STEP_1] Get Target Tweets (True Snapshot V3) \uC2DC\uC791");
  console.log("\u{1F4E1} \uC785\uB825:", JSON.stringify(event, null, 2));
  try {
    ...
    const collectionDate = event.targetDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const passiveRange = getSnapshotDateRange(SNAPSHOT_CONFIG.passive.daysAgo, collectionDate);
    const activeRange = getSnapshotDateRange(SNAPSHOT_CONFIG.active.daysAgo, collectionDate);
    console.log(`
\u{1F4CA} [TRUE_SNAPSHOT_V3] \uC2A4\uB0C5\uC0F7 \uC218\uC9D1 \uC804\uB7B5:`);
    console.log(`  \u{1F535} Passive (\uB204\uC801\uD615 \uC9C0\uD45C) - ${SNAPSHOT_CONFIG.passive.daysAgo}\uC77C \uC804:`);
    ...
    console.log(`  \u{1F7E2} Active (\uB300\uD654\uD615 \uC9C0\uD45C) - ${SNAPSHOT_CONFIG.active.daysAgo}\uC77C \uC804:`);
    ...
    console.log(`  \u{1F3AF} \uC218\uC9D1 \uB0A0\uC9DC: ${collectionDate}
`);
    console.log(`\u{1F464} \uD0C0\uAC9F \uC0AC\uC6A9\uC790 \uC815\uBCF4 \uAD6C\uC131: @${config.targetUsername} (${config.targetUserId})`);
    const targetUser = {
    ...
    if (!targetUser.id || targetUser.id === "12345") {
...
\u2705 X API \uC870\uD68C \uC644\uB8CC:`);
    ...
    console.log(`
\u{1F50D} \uC911\uBCF5 \uC218\uC9D1 \uBC29\uC9C0 \uCCB4\uD06C (SnapshotTracker):`);
    const uncollectedPassiveTweets = await snapshotTracker.filterUncollectedTweets(passiveTweets, "likes");
    console.log(`  \u{1F535} Passive: ${passiveTweets.length}\uAC1C \u2192 ${uncollectedPassiveTweets.length}\uAC1C (${passiveTweets.length - uncollectedPassiveTweets.length}\uAC1C\uB294 \uC774\uBBF8 \uC218\uC9D1\uB428)`);
    if (uncollectedPassiveTweets.length > 0) {
      console.log(`
\u{1F50D} [DEBUG] \uD544\uD130\uB9C1 \uC804 \uD2B8\uC717 \uC0C1\uD0DC (\uCD1D ${uncollectedPassiveTweets.length}\uAC1C):`);
      uncollectedPassiveTweets.forEach((tweet, idx) => {
        ...
        console.log(`     - referenced_tweets: ${JSON.stringify(tweet.referenced_tweets || [])}`);
        console.log(`     - text: "${tweet.text?.substring(0, 60)}..."`);
      });
    }
    const tweets = uncollectedPassiveTweets.filter((tweet) => !tweet.isReply).map((tweet) => ({
      ...tweet,
...
\u{1F4DD} Passive \uC218\uC9D1 \uB300\uC0C1 \uD2B8\uC717 \uBAA9\uB85D:`);
    ...
    console.log(`\u{1F3AF} \uD0C0\uAC9F \uD2B8\uC717 ID ${targetTweetIds.length}\uAC1C \uCD94\uCD9C\uB428 (\uC911\uBCF5 \uBC29\uC9C0\uC6A9)`);
    const result = {
      tweets,
      targetUser,
      dateRange: passiveRange,
      // 기본 범위 (하위 호환성)
      collectionDate,
      ...
      adaptiveWaitSeconds,
      targetTweetIds,
      // 🆕 타겟 트윗 ID 목록 (2025-10-26)
      snapshotStrategy: {
        // ⭐ V3: Passive/Active 구조
        passive: {
          ...passiveRange,
    ...
    console.log("\n\u2705 [STEP_1] Get Target Tweets (True Snapshot V3) \uC644\uB8CC:", {
      ...
      passiveOriginalPosts: tweets.length,
      // 모두 원본 포스트
    ...
    if (error.status === 429 || error.message?.includes("Rate limit")) {
      ...
      console.error("\u{1F6A8} [RATE_LIMIT] Rate Limit \uC5D0\uB7EC \uBC1C\uC0DD - 15\uBD84 \uB300\uAE30 \uD544\uC694");
...
//# sourceMappingURL=get-target-tweets.js.map
