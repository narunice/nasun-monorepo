...
var community_classifier_batch_exports = {};
__export(community_classifier_batch_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(community_classifier_batch_exports);
...
var LanguageAnalyzer = class {
  constructor(twitterApi2, config = {}) {
    this.twitterApi = twitterApi2;
...
var DeltaCalculator = class _DeltaCalculator {
  // 유효성 검증 통계
  constructor(dynamoClient2, tableName, communityService2, scoreWeights) {
   ...
   */
  setCommunityService(communityService2) {
    this.communityService = communityService2;
...
var CommunityClassificationService = class {
  constructor(dynamoClient2, twitterApi2, config = {}) {
    // 🆕 [Phase 3.3] 분류 통계 추적
    ...
    this.dynamoClient = import_lib_dynamodb2.DynamoDBDocumentClient.from(dynamoClient2);
    this.languageAnalyzer = new LanguageAnalyzer(twitterApi2, config);
  ...
  async clearCache(userId) {
    try {
      if (userId) {
        await this.dynamoClient.send(new import_lib_dynamodb2.UpdateCommand({
          TableName: this.config.tableName,
          Key: {
            pk: `USER_COMMUNITY#${userId}`,
            sk: "PROFILE"
...
var ReplyCounterService = class {
  constructor(tableName) {
    ...
    this.dynamoClient = import_lib_dynamodb3.DynamoDBDocumentClient.from(new import_client_dynamodb.DynamoDBClient({}));
    this.tableName = tableName;
  ...
  async atomicIncrementCounter(targetTweetId, userId) {
    ...
    try {
      try {
        ...
        await this.dynamoClient.send(new import_lib_dynamodb3.PutCommand(createParams));
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb3.UpdateCommand(updateParams));
  ...
  async markMaxReached(pk, sk) {
    try {
      ...
      await this.dynamoClient.send(new import_lib_dynamodb3.UpdateCommand(updateParams));
  ...
  async saveReplyEngagement(targetTweetId, userId, username, replyTweetId, replyText, conversationId, targetDate, sequence) {
    ...
    await this.dynamoClient.send(new import_lib_dynamodb3.PutCommand(putParams));
  ...
  async getReplyCount(targetTweetId, userId) {
    ...
    try {
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb3.QueryCommand(queryParams));
      if (result.Items && result.Items.length > 0) {
        const item = result.Items[0];
        return {
...
var MentionCounterService = class {
  constructor(tableName) {
    this.auditLogger = createAuditLogger();
    this.dynamoClient = import_lib_dynamodb4.DynamoDBDocumentClient.from(new import_client_dynamodb2.DynamoDBClient({}));
    this.tableName = tableName;
  ...
  async atomicIncrementWithCooldown(userId, targetDate) {
    ...
    try {
      try {
        ...
        await this.dynamoClient.send(new import_lib_dynamodb4.PutCommand(createParams));
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb4.UpdateCommand(updateParams));
      if (result.Attributes) {
  ...
  async getExistingRecord(pk, sk) {
    try {
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb4.QueryCommand(queryParams));
      if (result.Items && result.Items.length > 0) {
  ...
  async markMaxReached(pk, sk) {
    try {
      ...
      await this.dynamoClient.send(new import_lib_dynamodb4.UpdateCommand(updateParams));
  ...
  async saveMentionEngagement(userId, username, tweetId, tweetText, mentionedUserId, mentionedUsername, targetDate, sequence, intervalHours) {
    ...
    await this.dynamoClient.send(new import_lib_dynamodb4.PutCommand(putParams));
...
var QuoteCounterService = class {
  constructor(tableName) {
    this.auditLogger = createAuditLogger();
    this.dynamoClient = import_lib_dynamodb5.DynamoDBDocumentClient.from(new import_client_dynamodb3.DynamoDBClient({}));
    this.tableName = tableName;
  ...
  async validateQuoteAttempt(userId, targetDate) {
    try {
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb5.QueryCommand({
        TableName: this.tableName,
        KeyConditionExpression: "pk = :pk",
        ExpressionAttributeValues: {
          ":pk": pk
  ...
  async atomicIncrement(userId, targetDate) {
    try {
      ...
      const result = await this.dynamoClient.send(new import_lib_dynamodb5.UpdateCommand({
        TableName: this.tableName,
        Key: { pk, sk },
        UpdateExpression: `
          SET 
      ...
      if (updatedItem.quoteCount >= QUOTE_RULES.dailyLimit && !updatedItem.maxReachedAt) {
        await this.dynamoClient.send(new import_lib_dynamodb5.UpdateCommand({
          TableName: this.tableName,
          Key: { pk, sk },
  ...
  async saveQuoteEngagement(userId, username, quoteTweetId, quoteTweetText, originalTweetId, originalTweetText, sequence, intervalHours, targetDate) {
    try {
      ...
      await this.dynamoClient.send(new import_lib_dynamodb5.PutCommand({
        TableName: this.tableName,
        Item: engagementData,
...
// src/handlers/batch/community-classifier-batch.ts
var dynamoClient = import_lib_dynamodb6.DynamoDBDocumentClient.from(new import_client_dynamodb4.DynamoDBClient({
  region: process.env.AWS_REGION || "us-east-1"
}));
var secureTokenManager2 = new SecureTokenManager(process.env.AWS_REGION || "ap-northeast-2");
var twitterApi;
...
function loadConfig() {
  return {
    ...
    // 동시 처리 수 (Rate Limit 고려)
    maxUsers: parseInt(process.env.MAX_USERS_PER_RUN || "100"),
    // 한 번에 최대 처리할 사용자 수
    dryRun: process.env.DRY_RUN === "true",
    processingTimeoutMs: parseInt(process.env.PROCESSING_TIMEOUT_MS || "840000")
    // 14분 (15분 Lambda 타임아웃 - 1분 여유)
  };
}
async function initializeServices() {
  ...
  console.log(`\u{1F527} [INIT] Twitter API \uBC0F \uCEE4\uBBA4\uB2C8\uD2F0 \uC11C\uBE44\uC2A4 \uCD08\uAE30\uD654 \uC911...`);
  ...
  communityService = new CommunityClassificationService(
    new import_client_dynamodb4.DynamoDBClient({ region: process.env.AWS_REGION || "us-east-1" }),
    twitterApi
  );
  console.log(`\u2705 [INIT] \uC11C\uBE44\uC2A4 \uCD08\uAE30\uD654 \uC644\uB8CC`);
}
async function getActiveUsers(config) {
  console.log(`\u{1F465} [BATCH] \uD65C\uC131 \uC0AC\uC6A9\uC790 \uC218\uC9D1 \uC2DC\uC791 (\uCD5C\uB300 ${config.maxUsers}\uBA85)`);
  try {
    const thirtyDaysAgo = /* @__PURE__ */ new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    const cutoffDate = thirtyDaysAgo.toISOString();
    const result = await dynamoClient.send(new import_lib_dynamodb6.ScanCommand({
    ...
    if (!result.Items) {
      console.log(`\u{1F4ED} [BATCH] \uD65C\uC131 \uC0AC\uC6A9\uC790\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4`);
      return [];
    }
    const activeUsers = result.Items.filter((item) => item.userId && item.totalScore > 0).sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime()).map((item) => item.userId);
...
async function getUsersNeedingClassification(userIds, forceRefresh = false) {
  console.log(`\u{1F50D} [BATCH] \uBD84\uB958 \uD544\uC694 \uC0AC\uC6A9\uC790 \uD544\uD130\uB9C1 (\uAC15\uC81C\uC0C8\uB85C\uACE0\uCE68: ${forceRefresh})`);
  if (forceRefresh) {
    console.log(`\u{1F504} [BATCH] \uAC15\uC81C \uC0C8\uB85C\uACE0\uCE68 - \uBAA8\uB4E0 \uC0AC\uC6A9\uC790 \uBD84\uB958 \uB300\uC0C1`);
    return userIds;
  }
  const usersNeedingClassification = [];
...
var BatchProgressTracker = class {
  ...
  updateProgress(success) {
    ...
    if (progressPercent % 10 === 0 && this.processedUsers % Math.ceil(this.totalUsers / 10) === 0) {
      ...
      console.log(`\u{1F4CA} [BATCH] \uC9C4\uD589\uB960: ${progressPercent}% (${this.processedUsers}/${this.totalUsers}) \uC131\uACF5: ${this.successCount}, \uC2E4\uD328: ${this.errorCount}, \uC608\uC0C1 \uB0A8\uC740 \uC2DC\uAC04: ${Math.round(remainingMs / 1e3)}\uCD08`);
    }
  }
  getStats() {
    return {
...
async function sendCloudWatchMetrics(stats) {
  try {
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchProcessed: ${stats.totalProcessed}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchSuccess: ${stats.successCount}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchErrors: ${stats.errorCount}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchKoreanUsers: ${stats.koreanCount}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchGlobalUsers: ${stats.globalCount}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchCacheHitRatio: ${(stats.cacheHitCount / Math.max(stats.totalProcessed, 1) * 100).toFixed(1)}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchProcessingTime: ${stats.processingTimeMs}`);
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchAverageConfidence: ${(stats.averageConfidence * 100).toFixed(1)}`);
  } catch (error) {
...
var handler = async (event, context) => {
  ...
  console.log(`\u{1F680} [BATCH] \uCEE4\uBBA4\uB2C8\uD2F0 \uBD84\uB958 \uBC30\uCE58 \uCC98\uB9AC \uC2DC\uC791`);
  console.log(`\u{1F4CB} [BATCH] \uC124\uC815:`, {
  ...
  try {
    ...
    if (dryRun) {
      console.log(`\u{1F9EA} [BATCH] DRY RUN \uBAA8\uB4DC - \uC2E4\uC81C \uBD84\uB958\uB294 \uC218\uD589\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4`);
    }
    let userIds;
    if (targetUserIds && targetUserIds.length > 0) {
      console.log(`\u{1F3AF} [BATCH] \uC9C0\uC815\uB41C \uC0AC\uC6A9\uC790 ${targetUserIds.length}\uBA85 \uCC98\uB9AC`);
      userIds = targetUserIds.slice(0, maxUsers);
    } else {
      userIds = await getActiveUsers({ ...config, maxUsers });
    }
    if (userIds.length === 0) {
      console.log(`\u{1F4ED} [BATCH] \uCC98\uB9AC\uD560 \uC0AC\uC6A9\uC790\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4`);
      return {
        statusCode: 200,
    ...
    let stats;
    if (dryRun) {
      console.log(`\u{1F9EA} [BATCH] DRY RUN: ${usersToProcess.length}\uBA85 \uBD84\uB958 \uC2DC\uBBAC\uB808\uC774\uC158`);
      stats = {
        totalProcessed: usersToProcess.length,
        successCount: usersToProcess.length,
        errorCount: 0,
        cacheHitCount: 0,
        koreanCount: Math.floor(usersToProcess.length * 0.3),
        // 30% 가정
        globalCount: Math.floor(usersToProcess.length * 0.7),
        // 70% 가정
    ...
    console.log(`\u{1F389} [BATCH] \uCEE4\uBBA4\uB2C8\uD2F0 \uBD84\uB958 \uBC30\uCE58 \uCC98\uB9AC \uC644\uB8CC:`);
    console.log(`  - \uCD1D \uCC98\uB9AC \uC2DC\uAC04: ${Math.round(totalElapsedMs / 1e3)}\uCD08`);
    ...
    console.log(`\u{1F4CA} [METRIC] NASUN/Community/BatchError: 1`);
    return {
      statusCode: 500,
      body: JSON.stringify({
...
//# sourceMappingURL=community-classifier-batch.js.map
