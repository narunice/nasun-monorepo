...
var collect_retweets_exports = {};
__export(collect_retweets_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(collect_retweets_exports);
...
var RateLimitMonitor = class _RateLimitMonitor {
  ...
  async sendMetricsToCloudWatch() {
    ...
    try {
      const { CloudWatchClient: CloudWatchClient4, PutMetricDataCommand: PutMetricDataCommand4 } = await import("@aws-sdk/client-cloudwatch");
      const cloudWatchClient = new CloudWatchClient4({
      ...
      const command = new PutMetricDataCommand4({
...
var SnapshotTracker = class {
  constructor(tableName, region = "ap-northeast-2") {
    const dynamoClient = new import_client_dynamodb4.DynamoDBClient({ region });
    this.docClient = import_lib_dynamodb4.DynamoDBDocumentClient.from(dynamoClient);
    this.tableName = tableName;
  }
  /**
   ...
   */
  async isCollected(tweetId, engagementType) {
    try {
      const result = await this.docClient.send(new import_lib_dynamodb4.GetCommand({
        TableName: this.tableName,
        Key: {
   ...
   */
  async markAsCollected(tweetId, engagementType, metadata) {
    try {
      await this.docClient.send(new import_lib_dynamodb4.PutCommand({
        TableName: this.tableName,
        Item: {
          pk: "SNAPSHOT#COLLECTED",
          sk: `TWEET#${tweetId}#${engagementType.toUpperCase()}`,
   ...
   */
  async filterUncollectedTweets(tweets, engagementType) {
    if (tweets.length === 0) {
      return [];
    }
    console.log(`\u{1F50D} [SnapshotTracker] \uBBF8\uC218\uC9D1 \uD2B8\uC717 \uD544\uD130\uB9C1: ${tweets.length}\uAC1C \uD2B8\uC717 (\uD0C0\uC785: ${engagementType})`);
    const uncollectedTweets = [];
  ...
  async getCollectionsByDate(collectionDate) {
    try {
      ...
      return (result.Items || []).map((item) => ({
        ...
        engagementType: item.engagementType.toLowerCase(),
...
var handler = async (event) => {
  const startTime = Date.now();
  console.log("\u{1F504} [COLLECT_RETWEETS_V3] \uC2DC\uC791:", JSON.stringify(event, null, 2));
  try {
    ...
    const effectiveMaxResults = maxResults || config.maxRepostsPerTweet;
    console.log(`\u{1F4CA} [BATCH_${batchIndex}] ${tweets.length}\uAC1C \uD2B8\uC717\uC758 \uB9AC\uD2B8\uC717 \uC218\uC9D1 \uC2DC\uC791`);
    console.log(`\u2699\uFE0F [CONFIG] \uD2B8\uC717\uB2F9 \uCD5C\uB300 Repost \uC218\uC9D1\uB7C9: ${effectiveMaxResults}\uBA85 (\uD658\uACBD \uBCC0\uC218: ${config.maxRepostsPerTweet})`);
    if (priority) {
      console.log(`   \u{1F3AF} \uC6B0\uC120\uC21C\uC704: ${priority} | maxResults: ${effectiveMaxResults} | \uC804\uB7B5: ${collectionStrategy}`);
    }
    const tableName = process.env.CUMULATIVE_TABLE_NAME;
    if (!tableName) {
      throw new Error("CUMULATIVE_TABLE_NAME \uD658\uACBD\uBCC0\uC218\uAC00 \uC124\uC815\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4");
    }
    const dynamoClient = new import_client_dynamodb5.DynamoDBClient({ region: "ap-northeast-2" });
    const docClient = import_lib_dynamodb5.DynamoDBDocumentClient.from(dynamoClient);
    const snapshotTracker = new SnapshotTracker(tableName);
    for (const tweet of tweets) {
      try {
        console.log(`\u{1F504} [TWEET_${tweet.id}] \uB9AC\uD2B8\uC717 \uC218\uC9D1 \uC2DC\uC791`);
        const users = await twitterService.getTweetRepostedByUsers(tweet.id, effectiveMaxResults);
        apiCallsUsed++;
        console.log(`\u{1F4CA} [API_CALL] ${apiCallsUsed}\uBC88\uC9F8 \uD638\uCD9C \uC644\uB8CC`);
        if (users && users.length > 0) {
          for (const user of users) {
            const retweetEngagement = {
              tweet_id: tweet.id,
              engagement_type: "repost",
            ...
            retweetsCollected.push(retweetEngagement);
          }
          console.log(`\u2705 [TWEET_${tweet.id}] ${users.length}\uAC1C \uB9AC\uD2B8\uC717 \uC218\uC9D1 \uC644\uB8CC`);
          await snapshotTracker.markAsCollected(tweet.id, "retweets", {
            tweetCreatedAt: tweet.created_at,
            collectedAt: (/* @__PURE__ */ new Date()).toISOString(),
            daysElapsed: SNAPSHOT_CONFIG.passive.daysAgo,
            engagementCount: users.length,
            collectionDate: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
          });
          console.log(`\u{1F512} [TWEET_${tweet.id}] \uC218\uC9D1 \uC644\uB8CC \uB9C8\uD0B9 (Passive - Retweets)`);
        } else {
          const expectedRetweetCount = tweet.public_metrics?.retweet_count || 0;
          if (expectedRetweetCount > 0) {
            console.error(`\u26A0\uFE0F [VALIDATION_ERROR] [TWEET_${tweet.id}] Data mismatch detected!`);
            console.error(`   Expected retweets (public_metrics): ${expectedRetweetCount}`);
            console.error(`   Actual retweets (API response): 0`);
            console.error(`   \u26A0\uFE0F Possible causes: OAuth permission issue, API error, or rate limit`);
            console.error(`   \u{1F6AB} NOT marking as collected to allow retry`);
            throw new Error(`Data validation failed: public_metrics.retweet_count=${expectedRetweetCount} but API returned 0 retweets. Possible OAuth permission or API error.`);
          }
          console.log(`\u2139\uFE0F [TWEET_${tweet.id}] \uB9AC\uD2B8\uC717 \uC5C6\uC74C (public_metrics\uB3C4 0\uAC1C\uB85C \uD655\uC778)`);
          await snapshotTracker.markAsCollected(tweet.id, "retweets", {
            tweetCreatedAt: tweet.created_at,
            collectedAt: (/* @__PURE__ */ new Date()).toISOString(),
            daysElapsed: SNAPSHOT_CONFIG.passive.daysAgo,
          ...
          console.log(`\u{1F512} [TWEET_${tweet.id}] \uC218\uC9D1 \uC644\uB8CC \uB9C8\uD0B9 (Passive - Retweets, 0\uAC1C)`);
        }
      } catch (error) {
        console.error(`\u274C [TWEET_${tweet.id}] \uB9AC\uD2B8\uC717 \uC218\uC9D1 \uC2E4\uD328:`, error);
    ...
    const result = {
      ...
      batchIndex,
      retweetsCollected,
      totalRetweets: retweetsCollected.length,
      apiCallsUsed,
      processingDuration: Date.now() - startTime,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    };
    try {
      const cloudWatch = new import_client_cloudwatch3.CloudWatchClient({ region: "ap-northeast-2" });
      await cloudWatch.send(new import_client_cloudwatch3.PutMetricDataCommand({
        Namespace: "NASUN/StepFunctions/CollectRetweets",
        MetricData: [
          {
            MetricName: "RetweetsCollected",
            Value: retweetsCollected.length,
            Unit: "Count",
            Timestamp: /* @__PURE__ */ new Date()
          },
          {
    ...
    console.log(`\u2705 [COLLECT_RETWEETS_V3] \uBC30\uCE58 ${batchIndex} \uC644\uB8CC: ${retweetsCollected.length}\uAC1C \uB9AC\uD2B8\uC717 \uC218\uC9D1`);
    console.log("\u{1F4CB} [RESULT]:", JSON.stringify(result, null, 2));
    return result;
  } catch (error) {
    console.error("\u274C [COLLECT_RETWEETS_V3] \uC624\uB958 \uBC1C\uC0DD:", error);
    try {
      const cloudWatch = new import_client_cloudwatch3.CloudWatchClient({ region: "ap-northeast-2" });
      await cloudWatch.send(new import_client_cloudwatch3.PutMetricDataCommand({
        Namespace: "NASUN/StepFunctions/CollectRetweets",
...
//# sourceMappingURL=collect-retweets.js.map
