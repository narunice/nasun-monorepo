...
var collect_high_engagement_replies_exports = {};
__export(collect_high_engagement_replies_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(collect_high_engagement_replies_exports);
...
var handler = async (event) => {
  const startTime = Date.now();
  console.log("\u{1F4AC} [COLLECT_HIGH_ENGAGEMENT_REPLIES] \uC2DC\uC791:", JSON.stringify(event, null, 2));
  try {
    if (!event.tweets || !Array.isArray(event.tweets)) {
      throw new Error("\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC785\uB825: tweets \uBC30\uC5F4\uC774 \uD544\uC694\uD569\uB2C8\uB2E4");
    }
    const { tweets, targetUser } = event;
    const highEngagementTweets = tweets.filter((tweet) => (tweet.public_metrics?.like_count || 0) >= 100).sort((a, b) => (b.public_metrics?.like_count || 0) - (a.public_metrics?.like_count || 0)).slice(0, 3);
    console.log(`\u{1F4CA} [FILTER] ${tweets.length}\uAC1C \uC911 ${highEngagementTweets.length}\uAC1C \uB192\uC740 \uC778\uAC8C\uC774\uC9C0\uBA3C\uD2B8 \uD3EC\uC2A4\uD2B8 \uBC1C\uACAC`);
    ...
    for (const tweet of highEngagementTweets) {
      try {
        ...
        console.log(`\u{1F4AC} [TWEET_${tweet.id}] \uB2F5\uAE00 \uC218\uC9D1 \uC2DC\uC791 (\uC88B\uC544\uC694: ${likeCount}\uAC1C)`);
...
//# sourceMappingURL=collect-high-engagement-replies.js.map
