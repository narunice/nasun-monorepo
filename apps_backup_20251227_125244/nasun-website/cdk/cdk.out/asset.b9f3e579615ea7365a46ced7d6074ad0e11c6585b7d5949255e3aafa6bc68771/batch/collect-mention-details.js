...
var collect_mention_details_exports = {};
__export(collect_mention_details_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(collect_mention_details_exports);
...
// src/handlers/batch/collect-mention-details.ts
function isTargetAccountQuoteTweet(tweet, targetTweetIds) {
...
var handler = async (event) => {
  const startTime = Date.now();
  console.log("\u{1F50D} [COLLECT_MENTION_DETAILS_V3] \uC2DC\uC791:", JSON.stringify(event, null, 2));
  const ddbClient = new import_client_dynamodb4.DynamoDBClient({});
  const docClient = import_lib_dynamodb4.DynamoDBDocumentClient.from(ddbClient);
  try {
    ...
    console.log(`\u{1F3AF} \uD0C0\uAC9F \uD2B8\uC717 ID ${targetTweetIds.length}\uAC1C \uB85C\uB4DC\uB428 (\uC911\uBCF5 \uBC29\uC9C0\uC6A9)`);
    ...
    console.log(`\u{1F4E6} [BATCH_${mentionBatch.batchIndex}] ${mentionBatch.tweets.length}\uAC1C \uBA58\uC158 \uCC98\uB9AC \uC2DC\uC791`);
    let validMentions = 0;
    let rejectedMentions = 0;
    ...
    const allAuthorIds = mentionBatch.tweets.map((t) => t.author_id).filter((id) => !!id);
    console.log(`\u{1F465} ${allAuthorIds.length}\uBA85\uC758 \uC791\uC131\uC790 \uD504\uB85C\uD544 \uC77C\uAD04 \uC870\uD68C \uC2DC\uC791...`);
    ...
    for (const tweet of mentionBatch.tweets) {
      try {
        ...
        if (isTargetAccountQuoteTweet(tweet, targetTweetIds)) {
          console.log(`\u{1F504} [SKIP] Quote Tweet \uAC10\uC9C0 (Passive\uC5D0\uC11C \uC218\uC9D1\uB428): ${tweet.id}`);
          rejectedMentions++;
          continue;
        }
...
//# sourceMappingURL=collect-mention-details.js.map
