...
var collect_mentions_exports = {};
__export(collect_mentions_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(collect_mentions_exports);
...
var handler = async (event, context) => {
  const startTime = Date.now();
  console.log(`\u{1F680} [MENTIONS] Collect Mentions \uC2DC\uC791`);
  console.log("\uC804\uCCB4 \uC774\uBCA4\uD2B8:", JSON.stringify(event, null, 2));
  ...
  const activeRange = getSnapshotDateRange(SNAPSHOT_CONFIG.active.daysAgo);
  if (!event.collectionDate) {
    event.collectionDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    console.log("\u26A0\uFE0F collectionDate\uAC00 \uC5C6\uC5B4 \uAE30\uBCF8\uAC12\uC744 \uC124\uC815\uD569\uB2C8\uB2E4:", event.collectionDate);
  }
  console.log(`
\u{1F4CA} [TRUE_SNAPSHOT_V3] \uBA58\uC158 \uC218\uC9D1 \uC804\uB7B5:`);
  console.log(`  \u{1F7E2} Active (\uB300\uD654\uD615 \uC9C0\uD45C) - ${SNAPSHOT_CONFIG.active.daysAgo}\uC77C \uC804:`);
  console.log(`  \u{1F3AF} \uC218\uC9D1 \uD0C0\uC785: ${SNAPSHOT_CONFIG.active.types.join(", ")}`);
  console.log(`  \u{1F4C5} \uC774\uC720: ${SNAPSHOT_CONFIG.active.reason}`);
  console.log(`  \u23F0 \uBC94\uC704: ${activeRange.start} ~ ${activeRange.end}`);
  console.log(`  \u{1F512} \uC911\uBCF5 \uBC29\uC9C0: SnapshotTracker \uBA71\uB4F1\uC131 \uCCB4\uD06C
`);
  console.log("\u{1F4E1} \uC785\uB825:", JSON.stringify({
  ...
  try {
    ...
    console.log(`\u{1F522} [MENTIONS] \uBA58\uC158 \uCE74\uC6B4\uD130 \uC11C\uBE44\uC2A4 \uCD08\uAE30\uD654 \uC644\uB8CC`);
    console.log(`\u{1F512} [MENTIONS] SnapshotTracker \uCD08\uAE30\uD654 \uC644\uB8CC (\uBA71\uB4F1\uC131 \uBCF4\uC7A5)`);
    console.log(`\u{1F4DE} [MENTIONS] \uC218\uC9D1 \uC2DC\uC791 - \uC0AC\uC6A9\uC790: @${targetUser.username} (${targetUser.id})`);
    console.log(`\u{1F4C5} [MENTIONS] \uC218\uC9D1 \uAE30\uAC04 (1\uC77C \uC804 \uC2A4\uB0C5\uC0F7): ${activeRange.start} ~ ${activeRange.end}`);
    ...
    console.log(`\u{1F4DE} [MENTIONS] X API \uC218\uC9D1 \uC644\uB8CC: ${allMentions.length}\uAC1C \uBA58\uC158`);
    const uncollectedMentions = [];
    let skippedCount = 0;
    ...
    console.log(`\u{1F50D} [MENTIONS] \uC911\uBCF5 \uC218\uC9D1 \uBC29\uC9C0: ${allMentions.length}\uAC1C \u2192 ${uncollectedMentions.length}\uAC1C (${skippedCount}\uAC1C\uB294 \uC774\uBBF8 \uC218\uC9D1\uB428)`);
    ...
    console.log(`\u{1F4DE} [MENTIONS] \u2705 \uC804\uCCB4 \uCC98\uB9AC \uC644\uB8CC: ${uncollectedMentions.length}\uAC1C \uBA58\uC158 (${mentionDuration}ms)`);
    const totalProcessingTime = Date.now() - startTime;
    console.log(`\u{1F4DE} [MENTIONS] \uC804\uCCB4 \uCC98\uB9AC \uC644\uB8CC: \uC218\uC9D1 ${mentionDuration}ms + \uC800\uC7A5 ${totalProcessingTime - mentionDuration}ms = ${totalProcessingTime}ms`);
    ...
    const estimatedApiCalls = Math.ceil(allMentions.length / 100) + 1;
    const result = {
      success: true,
      mentionCount: uncollectedMentions.length,
      mentions: uncollectedMentions,
      // ⭐ V3: 중복 제거된 멘션만 반환
    ...
    if (isRateLimitError(error)) {
      console.log("\u{1F6A8} [MENTIONS] Rate Limit \uAC10\uC9C0 - Step Functions Retry \uC815\uCC45 \uD65C\uC131\uD654");
...
//# sourceMappingURL=collect-mentions.js.map
