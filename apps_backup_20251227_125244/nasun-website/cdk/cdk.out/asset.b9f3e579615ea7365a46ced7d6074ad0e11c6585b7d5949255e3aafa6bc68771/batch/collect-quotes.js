...
var collect_quotes_exports = {};
__export(collect_quotes_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(collect_quotes_exports);
...
var handler = async (event) => {
  const startTime = Date.now();
  console.log("\u{1F4AC} [COLLECT_QUOTES_V3] \uC2DC\uC791:", JSON.stringify(event, null, 2));
  try {
    ...
    console.log(`\u{1F4CA} [BATCH_${batchIndex}] ${tweets.length}\uAC1C \uD2B8\uC717\uC758 \uC778\uC6A9 \uC218\uC9D1 \uC2DC\uC791`);
    if (priority) {
    ...
    for (const tweet of tweets) {
      try {
        console.log(`\u{1F4AC} [TWEET_${tweet.id}] \uC778\uC6A9 \uC218\uC9D1 \uC2DC\uC791`);
        ...
        if (quoteTweets && quoteTweets.length > 0) {
          for (const quoteTweet of quoteTweets) {
            ...
            let quotingProfileImageUrl;
            let quotingFollowersCount;
            ...
            const quoteEngagement = {
              ...
              engaging_tweet_lang: quoteTweet.lang,
              // ✅ X API lang 필드 추가 (언어 감지용)
          ...
          console.log(`\u{1F512} [TWEET_${tweet.id}] \uC218\uC9D1 \uC644\uB8CC \uB9C8\uD0B9 (Passive - Quotes)`);
        } else {
          ...
          console.log(`\u{1F512} [TWEET_${tweet.id}] \uC218\uC9D1 \uC644\uB8CC \uB9C8\uD0B9 (Passive - Quotes, 0\uAC1C)`);
        }
      } catch (error) {
...
//# sourceMappingURL=collect-quotes.js.map
