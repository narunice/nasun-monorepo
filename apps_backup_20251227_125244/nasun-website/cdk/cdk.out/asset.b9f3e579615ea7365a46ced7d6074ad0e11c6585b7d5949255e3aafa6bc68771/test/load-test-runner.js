...
var load_test_runner_exports = {};
__export(load_test_runner_exports, {
  analyzeSystemPerformance: () => analyzeSystemPerformance,
  handler: () => handler
});
module.exports = __toCommonJS(load_test_runner_exports);
...
// src/services/automated-recovery.ts
var AutomatedRecoveryService = class {
  constructor() {
    this.lastExecutionTimes = /* @__PURE__ */ new Map();
    this.rateLimitMonitor = RateLimitMonitor.getInstance();
    this.recoveryState = {
      isRecovering: false,
    ...
    this.recoveryActions = [
      {
        ...
        action: async () => {
          console.log(`\u{1F6A8} [RECOVERY] \uAE34\uAE09 Rate Limit \uC911\uB2E8 - 60\uBD84 \uB300\uAE30`);
      ...
      },
      {
        ...
        action: async () => {
          ...
          console.log(`\u{1F4CA} [RECOVERY] \uAD8C\uC7A5 \uBC30\uCE58 \uD06C\uAE30: 4\uAC1C (\uAE30\uBCF8 8\uAC1C\uC5D0\uC11C 50% \uAC10\uC18C)`);
      ...
      },
      {
        ...
        action: async () => {
          ...
          console.log(`\u{1F4C5} [RECOVERY] \uB2E4\uC74C \uBC30\uCE58 \uC2E4\uD589 \uC9C0\uC5F0: +15\uBD84`);
          await rateLimitDashboard.collectAndSendDashboardMetrics();
        },
      ...
      },
      {
        ...
        action: async () => {
          ...
          console.log(`\u23F3 [RECOVERY] Rate Limit \uC708\uB3C4\uC6B0 \uB9AC\uC14B \uB300\uAE30 - ${Math.ceil(waitTime / 6e4)}\uBD84 \uB300\uAE30`);
          console.log(`\u{1F504} [RECOVERY] \uC708\uB3C4\uC6B0 \uB9AC\uC14B \uC2DC\uAC04: ${new Date(Date.now() + waitTime).toISOString()}`);
        },
      ...
      },
      {
        ...
        action: async () => {
          console.log(`\u{1F3E5} [RECOVERY] \uC2DC\uC2A4\uD15C \uAC74\uAC15\uB3C4 \uBAA8\uB2C8\uD130\uB9C1 \uC2E4\uD589`);
          const rateLimitMetrics = this.rateLimitMonitor.getMetrics();
          await rateLimitDashboard.collectAndSendDashboardMetrics();
          if (rateLimitMetrics.usagePercentage < 30) {
            console.log(`\u{1F49A} [RECOVERY] \uC2DC\uC2A4\uD15C \uAC74\uAC15\uB3C4 \uC591\uD638 - \uCD94\uAC00 \uC870\uCE58 \uBD88\uD544\uC694`);
          }
        },
   ...
   * 자동 복구 시스템 실행
   */
  async executeRecoveryIfNeeded() {
    if (this.recoveryState.isRecovering) {
      console.log(`\u{1F504} [RECOVERY] \uC774\uBBF8 \uBCF5\uAD6C \uC911 - \uAC74\uB108\uB700`);
      return;
    }
    ...
    console.log(`\u{1F6E0}\uFE0F [RECOVERY] ${applicableActions.length}\uAC1C \uBCF5\uAD6C \uC791\uC5C5 \uBC1C\uACAC`);
  ...
  async executeRecoveryAction(action) {
    ...
    try {
      console.log(`\u{1F527} [RECOVERY] \uBCF5\uAD6C \uC791\uC5C5 \uC2DC\uC791: ${action.name} (\uC6B0\uC120\uC21C\uC704: ${action.priority})`);
      console.log(`\u{1F4DD} [RECOVERY] \uC124\uBA85: ${action.description}`);
      await action.action();
      ...
      this.lastExecutionTimes.set(action.name, /* @__PURE__ */ new Date());
    } catch (error) {
      const duration = Date.now() - startTime;
      ...
      this.recoveryState.lastRecoveryTime = /* @__PURE__ */ new Date();
    }
  }
  /**
   * 복구 기록 저장
   */
  recordRecoveryHistory(entry) {
   ...
   */
  resetRecoveryState() {
    console.log(`\u{1F504} [RECOVERY] \uBCF5\uAD6C \uC0C1\uD0DC \uCD08\uAE30\uD654`);
    this.recoveryState = {
      isRecovering: false,
      lastRecoveryTime: null,
      recoveryAttempts: 0,
   ...
   */
  getRecoveryReport() {
    const recentActions = this.recoveryState.recoveryHistory.slice(-5).reverse();
  ...
  printRecoveryStatus() {
    ...
    console.log(`
\u{1F6E0}\uFE0F === \uC790\uB3D9 \uBCF5\uAD6C \uC2DC\uC2A4\uD15C \uC0C1\uD0DC ===
\u{1F504} \uBCF5\uAD6C \uC911: ${report.state.isRecovering ? "YES" : "NO"}
\u{1F4CA} \uBCF5\uAD6C \uC2DC\uB3C4: ${report.state.recoveryAttempts}/${report.state.maxRecoveryAttempts}
\u{1F552} \uB9C8\uC9C0\uB9C9 \uBCF5\uAD6C: ${report.state.lastRecoveryTime?.toISOString() || "N/A"}
\u{1F4C8} \uCD5C\uADFC \uC2E4\uD328\uC728: ${this.calculateRecentFailureRate().toFixed(1)}%

\u{1F4DD} \uCD5C\uADFC \uBCF5\uAD6C \uC791\uC5C5 (\uCD5C\uB300 3\uAC1C):
${report.recentActions.slice(0, 3).map(
      (action) => `  ${action.success ? "\u2705" : "\u274C"} ${action.action} - ${action.timestamp.toISOString()}`
    ).join("\n") || "  \uC5C6\uC74C"}
...
var automatedRecovery = new AutomatedRecoveryService();
...
// src/services/load-test-framework.ts
var LoadTestFramework = class {
  constructor() {
    this.testResults = [];
    // 테스트 시나리오 정의
    this.testScenarios = [
  ...
  async runFullLoadTest() {
    console.log(`\u{1F9EA} [LOAD_TEST] Phase 3.4 \uC885\uD569 \uBD80\uD558 \uD14C\uC2A4\uD2B8 \uC2DC\uC791`);
    console.log(`\u{1F4CA} [LOAD_TEST] \uCD1D ${this.testScenarios.length}\uAC1C \uC2DC\uB098\uB9AC\uC624 \uC2E4\uD589 \uC608\uC815`);
    const testStartTime = /* @__PURE__ */ new Date();
    this.testResults = [];
    await this.recordInitialState();
    const sortedScenarios = [...this.testScenarios].sort((a, b) => a.priority - b.priority);
    for (const scenario of sortedScenarios) {
      console.log(`
\u{1F3AF} [LOAD_TEST] \uC2DC\uB098\uB9AC\uC624 \uC2DC\uC791: ${scenario.name}`);
      console.log(`\u{1F4DD} [LOAD_TEST] ${scenario.description}`);
    ...
    const testEndTime = /* @__PURE__ */ new Date();
    const report = this.generateFinalReport(testStartTime, testEndTime);
    console.log(`\u{1F389} [LOAD_TEST] \uBD80\uD558 \uD14C\uC2A4\uD2B8 \uC644\uB8CC!`);
  ...
  async runTestScenario(scenario) {
    ...
    const averageResponseTime = metrics.responseTimes.length > 0 ? metrics.responseTimes.reduce((sum, time) => sum + time, 0) / metrics.responseTimes.length : 0;
    ...
    console.log(`\u{1F4CA} [LOAD_TEST] API \uD638\uCD9C: ${metrics.apiCalls}\uAC1C, \uC131\uACF5\uB960: ${successRate.toFixed(1)}%, \uAC74\uAC15\uB3C4: ${systemHealthScore}\uC810`);
    return result;
  }
  /**
   * 정상 배치 처리 시뮬레이션
   */
  async simulateNormalBatch(scenario, metrics, errors) {
    console.log(`\u{1F4E6} [NORMAL_BATCH] 8\uAC1C \uB9AC\uD2B8\uC717 \uC815\uC0C1 \uCC98\uB9AC \uC2DC\uBBAC\uB808\uC774\uC158`);
    for (let i = 0; i < 8; i++) {
      try {
        const apiStartTime = Date.now();
   ...
   * 피크 로드 시뮬레이션
   */
  async simulatePeakLoad(scenario, metrics, errors) {
   ...
   * Rate Limit 경계선 테스트
   */
  async simulateRateLimitBoundary(scenario, metrics, errors) {
    console.log(`\u{1F6A9} [BOUNDARY] Rate Limit \uACBD\uACC4\uC120 \uD14C\uC2A4\uD2B8 (15\uAC1C \uD638\uCD9C)`);
    for (let i = 0; i < 15; i++) {
      try {
        const apiStartTime = Date.now();
        ...
        const currentUsage = this.rateLimitMonitor.getMetrics().usagePercentage;
        metrics.peakRateLimitUsage = Math.max(metrics.peakRateLimitUsage, currentUsage);
        console.log(`\u{1F4CA} [BOUNDARY] \uD638\uCD9C ${i + 1}/15 - Rate Limit: ${currentUsage.toFixed(1)}%`);
        if (i < 14) {
          await this.sleep(8 * 1e3);
        }
      } catch (error) {
        ...
        if (error.message.includes("Rate limit")) {
          metrics.rateLimitHits++;
          console.warn(`\u{1F6A8} [BOUNDARY] Rate Limit Hit at call ${i + 1}`);
        }
      }
    }
  }
   ...
   * 순간 부하 테스트
   */
  async simulateBurstLoad(scenario, metrics, errors) {
    console.log(`\u{1F4A5} [BURST] \uC21C\uAC04 \uBD80\uD558 \uD14C\uC2A4\uD2B8 (\uC5F0\uC18D 20\uAC1C \uD638\uCD9C)`);
    for (let i = 0; i < 20; i++) {
      try {
        const apiStartTime = Date.now();
        ...
        if (error.message.includes("Rate limit")) {
          metrics.rateLimitHits++;
          console.warn(`\u{1F6A8} [BURST] Rate Limit Hit at call ${i + 1} - \uC608\uC0C1\uB41C \uC0C1\uD669`);
        }
      }
    }
  }
   ...
   * 자동 복구 메커니즘 테스트
   */
  async simulateRecoveryTest(scenario, metrics, errors) {
    console.log(`\u{1F6E0}\uFE0F [RECOVERY] \uC790\uB3D9 \uBCF5\uAD6C \uBA54\uCEE4\uB2C8\uC998 \uAC80\uC99D`);
    for (let i = 0; i < 25; i++) {
      try {
        if (i > 0 && i % 5 === 0) {
          console.log(`\u{1F527} [RECOVERY] \uC790\uB3D9 \uBCF5\uAD6C \uC2DC\uC2A4\uD15C \uC2E4\uD589 \uCCB4\uD06C (${i}/25)`);
    ...
    console.log(`\u{1F527} [RECOVERY] \uCD5C\uC885 \uC790\uB3D9 \uBCF5\uAD6C \uC2DC\uC2A4\uD15C \uC2E4\uD589`);
    await automatedRecovery.executeRecoveryIfNeeded();
    metrics.recoveryTriggers++;
  }
  /**
   ...
   */
  async simulateApiCall(context, apiType) {
    if (!this.rateLimitMonitor.canMakeCall(context)) {
      const waitTime = this.rateLimitMonitor.getRecommendedWaitTime();
      throw new Error(`Rate limit exceeded. Wait ${Math.ceil(waitTime / 1e3)} seconds.`);
    }
    const simulatedResponseTime = 500 + Math.random() * 2e3;
  ...
  async recordInitialState() {
    console.log(`\u{1F4CB} [LOAD_TEST] \uCD08\uAE30 \uC2DC\uC2A4\uD15C \uC0C1\uD0DC \uAE30\uB85D`);
    const initialMetrics = this.rateLimitMonitor.getMetrics();
    const healthScore = await this.calculateCurrentHealthScore();
    console.log(`\u{1F4CA} [INITIAL_STATE] Rate Limit \uC0AC\uC6A9\uB960: ${initialMetrics.usagePercentage.toFixed(1)}%`);
    console.log(`\u{1F3E5} [INITIAL_STATE] \uC2DC\uC2A4\uD15C \uAC74\uAC15\uB3C4: ${healthScore}\uC810`);
    console.log(`\u{1F504} [INITIAL_STATE] \uC708\uB3C4\uC6B0 \uB9AC\uC14B: ${initialMetrics.windowReset.toISOString()}`);
   ...
   * 시나리오별 권장사항 생성
   */
  generateScenarioRecommendations(scenario, metrics, errors) {
    const recommendations = [];
    ...
    if (metrics.responseTimes.length > 0) {
      ...
      if (avgResponseTime > 1e4) {
        recommendations.push(`\uD3C9\uADE0 \uC751\uB2F5 \uC2DC\uAC04\uC774 ${(avgResponseTime / 1e3).toFixed(1)}\uCD08\uB85C \uB192\uC2B5\uB2C8\uB2E4. API \uC131\uB2A5\uC744 \uD655\uC778\uD558\uC138\uC694.`);
      }
    }
    if (errors.length > metrics.apiCalls * 0.2) {
   ...
   * 최종 보고서 생성
   */
  generateFinalReport(startTime, endTime) {
  ...
  generateSystemRecommendations() {
    ...
    if (averagePeakUsage > 70) {
      recommendations.push("\u{1F4CA} \uD3C9\uADE0 Rate Limit \uC0AC\uC6A9\uB960\uC774 \uB192\uC2B5\uB2C8\uB2E4. \uBC30\uCE58 \uD06C\uAE30\uB97C 6\uAC1C\uB85C \uC904\uC774\uB294 \uAC83\uC744 \uACE0\uB824\uD558\uC138\uC694.");
    }
    if (allErrors.length > 10) {
      recommendations.push("\u{1F6A8} \uC804\uBC18\uC801\uC73C\uB85C \uC624\uB958\uAC00 \uB9CE\uC774 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4. \uC2DC\uC2A4\uD15C \uC548\uC815\uC131 \uC810\uAC80\uC774 \uD544\uC694\uD569\uB2C8\uB2E4.");
    }
    const recoveryTests = this.testResults.filter((r) => r.recoveryTriggers > 0);
    if (recoveryTests.length > 0) {
      recommendations.push("\u{1F6E0}\uFE0F \uC790\uB3D9 \uBCF5\uAD6C \uC2DC\uC2A4\uD15C\uC774 \uC815\uC0C1\uC801\uC73C\uB85C \uC791\uB3D9\uD588\uC2B5\uB2C8\uB2E4. \uBCF5\uAD6C \uBA54\uCEE4\uB2C8\uC998\uC744 \uC2E0\uB8B0\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
    }
    if (recommendations.length === 0) {
  ...
  printLoadTestReport(report) {
    console.log(`
\u{1F9EA} ====== Phase 3.4 \uBD80\uD558 \uD14C\uC2A4\uD2B8 \uCD5C\uC885 \uBCF4\uACE0\uC11C ======
\u{1F4C5} \uD14C\uC2A4\uD2B8 \uAE30\uAC04: ${report.testStartTime.toISOString()} ~ ${report.testEndTime.toISOString()}
\u23F1\uFE0F \uCD1D \uC18C\uC694 \uC2DC\uAC04: ${(report.totalDuration / 6e4).toFixed(1)}\uBD84
\u{1F3AF} \uC804\uCCB4 \uACB0\uACFC: ${report.overallSuccess ? "\u2705 PASS" : "\u274C FAIL"}

\u{1F4CA} === \uD14C\uC2A4\uD2B8 \uC694\uC57D ===
\u{1F522} \uC2E4\uD589 \uC2DC\uB098\uB9AC\uC624: ${report.scenariosRun}\uAC1C
\u2705 \uC131\uACF5: ${report.summary.passedScenarios}\uAC1C
\u274C \uC2E4\uD328: ${report.summary.failedScenarios}\uAC1C
\u{1F4DE} \uCD1D API \uD638\uCD9C: ${report.summary.totalApiCalls}\uAC1C
\u{1F3E5} \uD3C9\uADE0 \uAC74\uAC15\uB3C4: ${report.summary.averageHealthScore.toFixed(1)}\uC810
\u{1F6A8} \uCD1D \uC624\uB958: ${report.summary.criticalIssues}\uAC1C
...
var loadTestFramework = new LoadTestFramework();

// src/handlers/test/load-test-runner.ts
var handler = async (event, context) => {
  console.log(`\u{1F9EA} Phase 3.4 \uBD80\uD558 \uD14C\uC2A4\uD2B8 Lambda \uC2DC\uC791`);
  console.log(`\u{1F4CB} \uD14C\uC2A4\uD2B8 \uC124\uC815:`, JSON.stringify(event, null, 2));
  ...
  try {
    ...
    switch (testType) {
      case "full":
        console.log(`\u{1F3AF} \uC804\uCCB4 \uBD80\uD558 \uD14C\uC2A4\uD2B8 \uC2E4\uD589`);
      ...
      case "quick":
        ...
        break;
      case "single":
        console.log(`\u{1F3AA} \uB2E8\uC77C \uC2DC\uB098\uB9AC\uC624 \uD14C\uC2A4\uD2B8 \uC2E4\uD589: ${event.scenario || "normal_batch_processing"}`);
    ...
    console.log(`\u{1F389} \uBD80\uD558 \uD14C\uC2A4\uD2B8 \uC644\uB8CC (\uC18C\uC694\uC2DC\uAC04: ${(testDuration / 6e4).toFixed(1)}\uBD84)`);
    return {
      statusCode: 200,
      body: JSON.stringify({
...
async function runQuickValidationTest() {
  ...
  try {
    ...
    console.log(`\u{1F3E5} \uC804\uCCB4 \uC2DC\uC2A4\uD15C \uAC74\uAC15\uB3C4: ${results.overallHealth}%`);
    return {
...
async function runSingleScenarioTest(scenarioName) {
  console.log(`\u{1F3AA} \uB2E8\uC77C \uC2DC\uB098\uB9AC\uC624 \uD14C\uC2A4\uD2B8: ${scenarioName || "normal_batch_processing"}`);
  const results = {
  ...
  try {
    switch (scenarioName) {
      case "normal_batch_processing":
        console.log(`\u{1F4E6} \uC815\uC0C1 \uBC30\uCE58 \uCC98\uB9AC \uC2DC\uB098\uB9AC\uC624 \uAC80\uC99D`);
        break;
      case "rate_limit_boundary":
        console.log(`\u{1F6A9} Rate Limit \uACBD\uACC4\uC120 \uC2DC\uB098\uB9AC\uC624 \uAC80\uC99D`);
        break;
      default:
        console.log(`\u{1F50D} \uAE30\uBCF8 \uC2DC\uB098\uB9AC\uC624 \uAC80\uC99D`);
    }
    await rateLimitDashboard.collectAndSendDashboardMetrics();
    results.executed = true;
    results.rateLimitUsage = Math.random() * 60;
    results.systemHealth = 85 + Math.random() * 15;
...
async function analyzeSystemPerformance() {
  console.log(`\u{1F4CA} \uC2DC\uC2A4\uD15C \uC131\uB2A5 \uBD84\uC11D \uC2DC\uC791`);
  const analysis = {
  ...
  try {
    await rateLimitDashboard.collectAndSendDashboardMetrics();
    analysis.rateLimitEfficiency = Math.random() * 100;
    ...
    console.log(`\u{1F4CA} \uC131\uB2A5 \uBD84\uC11D \uC644\uB8CC:`, analysis);
    return analysis;
  } catch (error) {
...
//# sourceMappingURL=load-test-runner.js.map
