...
var get_rank_changes_exports = {};
__export(get_rank_changes_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_rank_changes_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    console.log(`\u{1F4CA} [getRankChanges] \uB7AD\uD0B9 \uBCC0\uB3D9 \uC870\uD68C \uC2DC\uC791: ${period}`);
    const config = getEnvConfigV2();
    ...
    console.log(`\u{1F4C5} [getRankChanges] \uC5B4\uC81C \uB0A0\uC9DC: ${yesterdayDate}`);
    ...
    const yesterdayEntries = yesterdayResult.success && yesterdayResult.data ? yesterdayResult.data.entries : [];
    console.log(`\u{1F4CA} [getRankChanges] \uB370\uC774\uD130 \uC870\uD68C \uC644\uB8CC:`, {
      today: todayResult.data.entries.length,
      yesterday: yesterdayEntries.length
    });
    const yesterdayMap = /* @__PURE__ */ new Map();
    ...
    for (const todayEntry of todayResult.data.entries) {
      ...
      const scoreChange = yesterday ? todayEntry.totalScore - yesterday.score : 0;
      rankChanges.push({
...
//# sourceMappingURL=get-rank-changes.js.map
