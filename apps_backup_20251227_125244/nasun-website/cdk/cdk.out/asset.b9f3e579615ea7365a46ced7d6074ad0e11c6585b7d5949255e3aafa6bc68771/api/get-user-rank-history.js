...
var get_user_rank_history_exports = {};
__export(get_user_rank_history_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_user_rank_history_exports);
...
var LeaderboardPeriod = /* @__PURE__ */ ((LeaderboardPeriod2) => {
  LeaderboardPeriod2["CUMULATIVE"] = "CUMULATIVE";
  LeaderboardPeriod2["EVENT1"] = "EVENT1";
  LeaderboardPeriod2["EVENT2"] = "EVENT2";
  LeaderboardPeriod2["EVENT3"] = "EVENT3";
  return LeaderboardPeriod2;
})(LeaderboardPeriod || {});
function getEventPeriodConfigs() {
  const config2 = getEnvConfigV2();
  return {
    ...
    ["EVENT1" /* EVENT1 */]: {
      ...
      startDate: config2.event1StartDate,
      endDate: config2.event1EndDate
    },
    ["EVENT2" /* EVENT2 */]: {
      period: "EVENT2" /* EVENT2 */,
      name: "2\uCC28 \uC774\uBCA4\uD2B8",
      description: "2\uCC28 \uC774\uBCA4\uD2B8 \uAE30\uAC04 (\uD658\uACBD\uBCC0\uC218 \uAE30\uBC18)",
      startDate: config2.event2StartDate,
      endDate: config2.event2EndDate
    },
    ["EVENT3" /* EVENT3 */]: {
      period: "EVENT3" /* EVENT3 */,
      name: "3\uCC28 \uC774\uBCA4\uD2B8",
      description: "3\uCC28 \uC774\uBCA4\uD2B8 \uAE30\uAC04 (\uD658\uACBD\uBCC0\uC218 \uAE30\uBC18)",
      startDate: config2.event3StartDate,
      endDate: config2.event3EndDate
    }
  };
}
...
function isAccountExcluded(username, userId, config2) {
  if (!config2) {
    ...
    config2 = parseResult.config;
  }
  if (config2.excludedUsernames.includes(username)) {
    return {
      isExcluded: true,
  ...
  if (config2.excludedUserIds.includes(userId)) {
    return {
      isExcluded: true,
...
function isAdminUser(username, config2) {
  if (!config2) {
    const parseResult = loadExcludedAccountsConfig();
    if (!parseResult.success) {
  ...
  const isAdmin = config2.adminUsernames.includes(username);
  return {
    isAdmin,
...
function validateExcludedAccountsConfig(config2) {
  try {
    if (!config2 || typeof config2 !== "object") {
      return false;
    }
    if (!Array.isArray(config2.excludedUsernames) || !Array.isArray(config2.excludedUserIds) || !Array.isArray(config2.adminUsernames)) {
      return false;
    }
    if (config2.adminUsernames.length === 0) {
...
var LeaderboardService = class {
  constructor(ddbClient2, config2) {
...
// src/handlers/api/get-user-rank-history.ts
var ddbClient = new import_client_dynamodb2.DynamoDBClient({ region: process.env.AWS_REGION || "ap-northeast-2" });
var config = getEnvConfigV2();
var leaderboardService = new LeaderboardService(ddbClient, config);
var CORS_HEADERS = {
...
var handler = async (event) => {
  ...
  console.log("\u{1F4CA} [get-user-rank-history] \uC694\uCCAD \uC2DC\uC791", {
    pathParameters: event.pathParameters,
    queryStringParameters: event.queryStringParameters
  });
  try {
    const period = event.pathParameters?.period?.toUpperCase();
    ...
    console.log("\u{1F4C5} [get-user-rank-history] \uB0A0\uC9DC \uBC94\uC704 \uACC4\uC0B0", {
      days,
...
function calculateStats(history, currentRank) {
  if (history.length === 0) {
    return {
  ...
  const rankImprovement = firstRank - lastRank;
  return {
    bestRank,
    worstRank,
...
//# sourceMappingURL=get-user-rank-history.js.map
