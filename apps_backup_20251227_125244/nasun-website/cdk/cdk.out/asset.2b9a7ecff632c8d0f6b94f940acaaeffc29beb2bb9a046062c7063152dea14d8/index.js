...
const dynamoClient = lib_dynamodb_1.DynamoDBDocumentClient.from(client);
...
const handler = async (event) => {
    ...
    try {
        if (isUnlink) {
            ...
            const getPrimaryCommand = new lib_dynamodb_1.GetCommand({
                TableName: tableName,
            ...
            if (providerKey === 'google') {
                ...
                // If unlinking MetaMask, remove wallet address
            ...
            const updatePrimaryCommand = new lib_dynamodb_1.UpdateCommand({
                TableName: tableName,
                Key: { identityId: primaryIdentityId },
            ...
            if (secondaryIdentityId) {
                const getSecondaryCommand = new lib_dynamodb_1.GetCommand({
                    TableName: tableName,
                ...
                if (secondaryProfile?.linkedAccounts) {
                    ...
                    const updateSecondaryCommand = new lib_dynamodb_1.UpdateCommand({
                        TableName: tableName,
                        Key: { identityId: secondaryIdentityId },
        ...
        const primaryProviderKey = primaryProfile.provider?.toLowerCase() || 'unknown';
        const reverseLinkInfo = {
            identityId: primaryIdentityId,
            username: primaryProfile.username || 'N/A',
            linkedAt: new Date().toISOString(),
        };
        if (primaryProfile.email)
            reverseLinkInfo.email = primaryProfile.email;
        if (primaryProfile.twitterHandle)
            reverseLinkInfo.twitterHandle = primaryProfile.twitterHandle;
        if (primaryProfile.twitterId)
            reverseLinkInfo.twitterId = primaryProfile.twitterId;
        if (primaryProfile.profileImageUrl)
            reverseLinkInfo.profileImageUrl = primaryProfile.profileImageUrl;
        // MetaMask-specific field
        if (primaryProfile.walletAddress)
            reverseLinkInfo.walletAddress = primaryProfile.walletAddress;
