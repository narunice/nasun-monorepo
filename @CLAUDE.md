...
| `apps/nasun-website`    | @nasun/nasun-website    | ✅ 완료 | EC2 스크립트 | 공식 웹사이트 (거버넌스)     |
| `apps/gensol-website`   | @nasun/gensol-website   | ✅ 완료 | EC2 스크립트 | GenSol 웹사이트 (NFT IP)     |
| `apps/pado`             | @nasun/pado             | ✅ 완료 | -            | Pado 앱 (통합 금융)          |
...
│   ├── nasun-website/             # @nasun/nasun-website - 공식 웹사이트 (거버넌스, 리더보드)
│   │   └── frontend/              # Vite React 앱
│   ├── gensol-website/            # @nasun/gensol-website - GenSol 웹사이트 (NFT IP)
│   │   └── frontend/              # Vite React 앱
│   └── pado/                      # @nasun/pado - Pado 앱 (통합 금융)
│       └── frontend/              # Vite React 앱
├── packages/
│   ├── wallet/                    # @nasun/wallet - 지갑 핵심 로직 (Signer, AA, Link)
...
### @nasun/wallet (Core Wallet Logic)

지갑 핵심 로직과 React hooks를 제공합니다. (P1 & P2 Completed)

**주요 기능 (Implemented):**
- **Signer Abstraction**: Local, zkLogin, EVM, Ledger 통합 서명 인터페이스
- **Multi-chain Support**: Sui (Move) 및 11개 EVM 체인 지원
- **WalletConnect v2**: dApp 연결 지원 (EIP-155, Sui namespace)
- **Account Abstraction**: ERC-4337 Smart Account, Paymaster (Gasless), Session Keys
- **Nasun Link v2**: URL 기반 토큰 전송 및 클레임
- **zkLogin**: Google, Apple, Twitch 등 소셜 로그인 지원
- **Payment UX**: 통합 결제 훅 및 QR 코드 생성

**사용법:**

```typescript
import { useWallet, useSigner, configureWallet } from "@nasun/wallet";
...
### @nasun/wallet-ui (UI Components)

지갑 관련 React UI 컴포넌트를 제공합니다.

**주요 컴포넌트:**
- `WalletProvider`: 컨텍스트 제공자
- `WalletConnect`: 지갑 연결/관리 모달 (Ledger, zkLogin, Mnemonic 포함)
- `BalanceDisplay`: 잔액 표시
- `SendTransaction`: 전송 UI
