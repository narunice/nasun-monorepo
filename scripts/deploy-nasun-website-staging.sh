#!/bin/bash
# ==============================================================================
# nasun-website 스테이징 배포 스크립트 (모노레포 버전)
# ==============================================================================
# 대상: staging.nasun.io
# 빌드: pnpm build:nasun-website --mode staging
# ==============================================================================
...
for arg in "$@"; do
  case $arg in
    --dry-run)
      ...
      ;;
    --help|-h)
      echo "사용법: ./scripts/deploy-nasun-website-staging.sh [옵션]"
      echo ""
      echo "옵션:"
...
echo ""
echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  🚀 NASUN Website 스테이징 배포                            ║${NC}"
echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
...
if ! pnpm --filter @nasun/nasun-website exec vite build --mode staging 2>&1; then
  log_error "빌드 실패!"
fi
