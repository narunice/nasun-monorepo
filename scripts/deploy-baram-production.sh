...
# ==============================================================================
# 대상: baram.nasun.io
# 빌드: pnpm build (from apps/baram/frontend)
# 기능: 백업, 롤백 지원
# ==============================================================================
...
# --- 설정 변수 ---
APP_NAME="baram"
APP_DIR="$MONOREPO_ROOT/apps/baram"

# EC2 설정 (Production)
PEM_KEY_PATH="$HOME/.ssh/nasun-prod-key.pem"
EC2_USER="ec2-user"
EC2_HOST="43.200.67.52"
REMOTE_DIR="/var/www/baram/dist"
BACKUP_DIR="/var/www/baram/backups"
HEALTH_CHECK_URL="https://baram.nasun.io"
...
for arg in "$@"; do
  case $arg in
    ...
    --rollback)
      ...
      ;;
    --help|-h)
      echo "사용법: ./scripts/deploy-baram-production.sh [옵션]"
      echo ""
      echo "옵션:"
...
echo ""
echo -e "${CYAN}========================================================${NC}"
echo -e "${CYAN}  Baram Production Deploy${NC}"
echo -e "${CYAN}  Target: ${YELLOW}baram.nasun.io${NC}"
echo -e "${CYAN}========================================================${NC}"
echo ""
...
if [ ! -f "$APP_DIR/.env" ]; then
  log_warning ".env 파일이 없습니다. VITE_ 환경변수가 빌드에 포함되지 않을 수 있습니다."
fi
...
if [ "$DRY_RUN" = false ] && [ "$FORCE" = false ]; then
  echo ""
  echo -e "${YELLOW}========================================================${NC}"
  echo -e "${YELLOW}  Production deploy confirmation${NC}"
  echo -e "${YELLOW}  This will affect the live site at baram.nasun.io${NC}"
  echo -e "${YELLOW}========================================================${NC}"
  echo ""
...
if ! (cd "$APP_DIR/frontend" && npx tsc --noEmit) 2>&1; then
...
if ! (cd "$APP_DIR/frontend" && npx vite build) 2>&1; then
  log_error "빌드 실패!"
fi
...
echo ""
echo -e "${GREEN}========================================================${NC}"
echo -e "${GREEN}  Baram production deploy complete!${NC}"
echo -e "${GREEN}  URL:     ${CYAN}https://baram.nasun.io${NC}"
echo -e "${GREEN}  Backup:  ${CYAN}${BACKUP_NAME}${NC}"
echo -e "${GREEN}  Elapsed: ${CYAN}$(get_elapsed_time $START_TIME)${NC}"
echo -e "${GREEN}  Rollback: ${CYAN}pnpm deploy:baram:prod -- --rollback${NC}"
echo -e "${GREEN}========================================================${NC}"
echo ""
