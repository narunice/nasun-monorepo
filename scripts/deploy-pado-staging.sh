#!/bin/bash
# ==============================================================================
# Pado 스테이징 배포 스크립트 (모노레포 버전)
# ==============================================================================
# 대상: staging.pado.finance
# 빌드: pnpm --filter @nasun/pado exec vite build --mode staging
# ==============================================================================
...
# EC2 설정 (Staging - nasun/gensol과 동일 서버)
PEM_KEY_PATH="$HOME/.ssh/.awskey/naru_seoul.pem"
EC2_USER="ubuntu"
EC2_HOST="ec2-15-165-19-180.ap-northeast-2.compute.amazonaws.com"
REMOTE_DIR="/var/www/staging.pado.finance"
BOTS_DIR="$MONOREPO_ROOT/apps/pado/bots"
BOTS_REMOTE_DIR="/home/ubuntu/pado-bots"
HEALTH_CHECK_URL="https://staging.pado.finance"
...
for arg in "$@"; do
  case $arg in
    ...
    --skip-bots)
      ...
      ;;
    --help|-h)
      echo "사용법: ./scripts/deploy-pado-staging.sh [옵션]"
      echo ""
      echo "옵션:"
...
echo ""
echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  🚀 Pado 스테이징 배포                                    ║${NC}"
echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
echo -e "${CYAN}║  대상: ${YELLOW}staging.pado.finance${CYAN}                              ║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f "$APP_DIR/.env.staging" ]; then
  log_warning ".env.staging 파일이 없습니다. .env.local로 대체합니다."
  if [ -f "$APP_DIR/.env.local" ]; then
    cp "$APP_DIR/.env.local" "$APP_DIR/.env.staging"
    log_success ".env.local -> .env.staging 복사 완료"
  else
    log_error ".env.local 파일도 없습니다. 환경 변수 파일이 필요합니다."
  fi
fi
...
if ! pnpm --filter @nasun/pado exec vite build --mode staging 2>&1; then
  log_error "빌드 실패!"
fi
...
if [ "$SKIP_BOTS" = false ]; then
  log_step 7 $TOTAL_STEPS "LP 봇 코드 동기화"
  ...
  log_step 8 $TOTAL_STEPS "LP 봇 PM2 시작"
